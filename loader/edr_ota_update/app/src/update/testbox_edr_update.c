...
#pragma bss_seg(".testbox_edr_update.data.bss")
#pragma data_seg(".testbox_edr_update.data")
#pragma const_seg(".testbox_edr_update.text.const")
#pragma code_seg(".testbox_edr_update.text")
#endif
...
static int bt_send_update_len(u32 update_len);
...
int bt_send_update_len(u32 update_len)
{
    u8 remin_cnt = 0;
re_send:
    bt_updata_clr_flag(UPDATA_REV_DATA);    //clr flag
...
void bt_updata_init_low_level()
{
    ...
    sys_timer_add(NULL, (void *)stack_run_loop_resume, 5);
    ...
    if (parameter_point) {
        mac_point = update_param_ext_get(parameter_point, EXT_BT_MAC_ADDR);
    }
    if (mac_point) {
        memcpy(bt_addr, mac_point, 6);
    } else {
...
static u32 connect_check_time = 0;
static void bt_update_connect_check(void *priv)
{
    connect_check_time ++;
    if (bt_updata_get_flag()&BIT(UPDATA_START)) {
        connect_check_time = 0;
    }
    if (connect_check_time > 10) {
...
void bt_update_state_cbk(u32 status, void *priv)
{
    ...
    case UPDATE_START:      //测试盒bt升级需要根据priv判断是否恢复基带
        ...
        sys_timer_add(NULL, bt_update_connect_check, 60000);
        /* bredr_bd_close_frame(); */
        /* bt_update_reinit(); */
#if defined (CONFIG_CPU_BR27) || defined (CONFIG_CPU_BR28) || defined (CONFIG_CPU_BR36)  //该分支只有27、28和36支持保持蓝牙
