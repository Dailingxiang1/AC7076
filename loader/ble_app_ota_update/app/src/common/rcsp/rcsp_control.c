...
#pragma bss_seg(".rcsp_control.data.bss")
#pragma data_seg(".rcsp_control.data")
#pragma const_seg(".rcsp_control.text.const")
#pragma code_seg(".rcsp_control.text")
#endif
...
#define UPDATE_AGAIN_RESULT_FLAG_BITMAP		0xFE // 用于先升级代码再升级资源的标志位
...
static u32 JL_opcode_get_target_info(void *priv, u8 OpCode, u8 OpCode_SN, u32 mask)
{
    ...
    if (mask & BIT(ATTR_TYPE_SDK_TYPE)) {
        ...
        log_info(">>>sdk_type:0x%x", sdk_type);
    }
    ...
    if (JL_rcsp_get_pvid_flag()) {
        if (mask & BIT(ATTR_TYPE_DEV_VID_PID)) {
            if (ex_cfg_get_content_api(CFG_ITEM_PVID, read_buf, 4) == EX_CFG_ERR_NONE) {
                log_info("ATTR_TYPE_DEV_VID_PID:");
                put_buf(read_buf, 4);
                u16 pvid[2] = {0};
        ...
        if (mask & BIT(ATTR_TYPE_DEV_AUTHKEY)) {
            if (ex_cfg_get_content_api(CFG_ITEM_VER_INFO_AUTHKEY, read_buf, 24) == EX_CFG_ERR_NONE) {
                log_info("ATTR_TYPE_DEV_AUTHKEY:");
                put_buf(read_buf, 24);
        ...
        if (mask & BIT(ATTR_TYPE_DEV_PROCODE)) {
            if (ex_cfg_get_content_api(CFG_ITEM_VER_INFO_PROCODE, read_buf, 24) == EX_CFG_ERR_NONE) {
                log_info("ATTR_TYPE_DEV_PROCODE:");
                put_buf(read_buf, 24);
    ...
    if (mask & BIT(ATTR_TYPE_DOUBLE_PARITION)) {
        extern u8 spceial_update_flag_get(void);
        if (spceial_update_flag_get()) {
            u8 update_parm[4] = {0, 0, 1, 1};
            offset += add_one_attr(buf, sizeof(buf), offset, ATTR_TYPE_DOUBLE_PARITION, update_parm, sizeof(update_parm));
        }
    }
#endif
...
#if defined(CONFIG_CPU_BR35)
        extern u8 spceial_update_flag_get(void);
        u8 update_status_value[3] = {0x1, 0, 0};
        update_status_value[2] = spceial_update_flag_get(); // 特殊升级标志位，用于先升级代码，再SDK中升级资源的情况
        log_info("ATTR_TYPE_UPDATE_STATUS:%d, %d\n", update_status_value[0], update_status_value[2]);
        offset += add_one_attr(buf, sizeof(buf), offset, ATTR_TYPE_UPDATE_STATUS, update_status_value, sizeof(update_status_value));
#else
...
static void JL_rcsp_cmd_resp(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    case JL_OPCODE_INQUIRE_DEVICE_IF_CAN_UPDATE:
        if (DEV_UPDATE_FILE_INFO_LEN == len) {
            ...
            log_info_hexdump(data, DEV_UPDATE_FILE_INFO_LEN);
            task_post_msg(NULL,
                          4,
...
/* { */
/*     rcsp_log_info("### rcsp_hid_data_send %d\n", len); */
/*     int err = 0; */
...
void rcsp_dev_select(u8 type)
{
    ...
    case RCSP_BLE:
        ...
        //log_info_hexdump((u8 *)&JL_pro_BLE_callback,sizeof(JL_PRO_CBK));
    ...
    case RCSP_HID:
        log_info("------RCSP_HID-----\n");
        rcsp_hid_callback_set(rcsp_receieve_data_handle, JL_rcsp_hid_status_callback);
        rcsp_ble_callback_set(NULL, NULL, NULL);
...
#if defined(CONFIG_CPU_BR35)
        if (JL_PRO_STATUS_SUCCESS == status && spceial_update_flag_get()) {
            status = UPDATE_AGAIN_RESULT_FLAG_BITMAP;
        }
#endif
...
void jl_rcsp_msg_deal(void *hdl, int *msg)
{
    ...
    //log_info("JL_RCSP msg[1]:%x,msg[2]:%x\n", msg[1], msg[2]);
    switch (msg[0]) {
...
#ifdef USB_HID_MODULE_CONTROL
    if (__this->rcsp_hid->send_data) {
        log_info("rcsp auth init---0\n");
        JL_rcsp_auth_init(__this->rcsp_hid->send_data, NULL, NULL);
    }
#else
    if (__this->rcsp_ble->send_data) {
        log_info("rcsp auth init----1\n");
