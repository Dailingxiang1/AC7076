...
volatile u8 *ai_txt2 = NULL;
volatile u8 *ai_txttempa = NULL;
volatile u8 *ai_txttempb = NULL;
u8 aidial_flag;
#define AI_INTERACTION_REC_LIMIT_MIN_MS		(1000) //录音时间最低限制
#define MAX_TXT_NUM                         (500)  //最大显示的字数
//截取UTF8编码第num位到末尾字符串
...
static char *getutf8stringend(u8 *str, int num)
{
    ...
    strncpy(substring, (char *)&str[i + 1], substringlength);
...
static u8 *getutf8stringfrontstring(u8 *str, int num)
{
    ...
    strncpy(substring, (char *)str, substringlength);
    substring[substringlength] = '\0';
    return (u8 *)substring;
}
...
static int ai_text_rx_cb(u8 ai_text, u8 *data, int len)
{
    ...
    memset((void *)ai_dial_txt1, '\0', len + 1);
    memcpy((void *)ai_dial_txt1, data, len);
    ai_dial_txt1[len] = '\0';
    u8 *str = getutf8stringfrontstring(data, MAX_TXT_NUM + 1);
    char *str1 = getutf8stringendstring(data, MAX_TXT_NUM);
