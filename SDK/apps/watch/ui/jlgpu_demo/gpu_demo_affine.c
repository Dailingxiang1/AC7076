...
#if defined GPU_DEMO_AFFINE_ENABLE && GPU_DEMO_AFFINE_ENABLE
...
{
    int i;
    for (i = 0; i < sizeof(affine_img_id_tab) / sizeof(affine_img_id_tab[0]); i++) {
        if (affine_img_id_tab[i] == image_id) {
            return i;
        }
    }
    return -1;
...
static void gpu_port_demo_create_affine_task(struct affine_info_user *affine_user)
{
    struct ui_image_attrs *image;
    int image_num;

    image = (struct ui_image_attrs *)malloc(affine_user->image_num * sizeof(struct ui_image_attrs));
    image_num = affine_user->image_num;
    char *img_path = affine_user->path;
    ...
    for (int i = 0; i < image_num; i++) {
        JLGPUTaskParam_t task_param = {0};
        printf("id:%x\n", affine_img_id_tab[i]);
        jlui_res_get_image_info(0, (affine_img_id_tab[i] & 0xffff), fp, &task_param.image, &task_param.info);
    ...
    float ratio_w = 0.5;
    float ratio_h = 0.5;
    gpu_matrix_t scale_matrix = {0};
    gpu_matrix_set_identity(&scale_matrix);
    gpu_matrix_translate(&scale_matrix, -win_rect.left, -win_rect.top);
    gpu_matrix_translate(&scale_matrix, -(win_rect.width - image[index].width * ratio_w) / 2, -(win_rect.height - image[index].height * ratio_h) / 2); /* 屏幕居中显示 */
    gpu_matrix_scale(&scale_matrix, 1.0f / ratio_w, 1.0f / ratio_h);

    //旋转45度(变换后的图片)
    gpu_matrix_t rotate_matrix = {0};
    gpu_matrix_set_identity(&rotate_matrix);
    gpu_matrix_translate(&rotate_matrix, -win_rect.left, -win_rect.top);
    gpu_matrix_translate(&rotate_matrix, -win_rect.width / 2, -win_rect.height / 2);
#define M_PI 3.1415926
    float angle = 45; // 角度
    float radian = -angle * M_PI / 180.0f;   // 角度转弧度
    gpu_matrix_rotate(&rotate_matrix, radian);
    gpu_matrix_translate(&rotate_matrix, win_rect.width / 2, win_rect.height / 2);

    //矩阵相乘，实现缩放平移和旋转的效果
    gpu_matrix_t mix_matrix = {0};
    gpu_matrix_translate(&scale_matrix, win_rect.left, win_rect.top);
    gpu_matrix_translate(&rotate_matrix, win_rect.left, win_rect.top);
    gpu_matrix_multiply(&mix_matrix, &scale_matrix, &rotate_matrix);//注意顺序：缩放在前， 旋转在后
    gpu_matrix_translate(&mix_matrix, -win_rect.left, -win_rect.top);
    ...
    if (index != -1) {
        ...
        gpu_port_demo_create_img_task(&img_user, &mix_matrix);
    }
    //时针
...
void gpu_port_demo_draw_affine(struct affine_info_user *affine_user, u32 background)
{
    //输出配置
    ...
    //创建绘制图片的任务
    gpu_port_demo_create_affine_task(affine_user);
...
void gpu_demo_affine(void *p)
{
    gpu_port_demo_lcd_init();		//初始化lcd
    gpu_port_demo_port_init();		//初始化gpu_port接口
    struct affine_info_user affine_user = {0};

    affine_user.path = "storage/virfat_flash/C/ui/ui.res";
    affine_user.image_num = sizeof(affine_img_id_tab) / sizeof(affine_img_id_tab[0]);
    affine_user.image_id_tab = affine_img_id_tab;
    affine_user.x_diff = 0;
    affine_user.y_diff = 0;

    gpu_port_demo_task_init(GPU_FORMAT_RGB565);
    gpu_port_demo_draw_affine(&affine_user, 0x64000000);//ARGB8888格式 alpha范围 : 0~100
...
int gpu_demo_affine_task(void)
{
    printf(">>>>>>>>> %s()", __func__);
    int err;
    err = os_task_create(gpu_demo_affine, NULL, 10, 1024 * 5, 512, "gpu_demo_affine");
    if (err != OS_NO_ERR) {
