...
#if defined GPU_DEMO_FLASH_565_ENABLE && GPU_DEMO_FLASH_565_ENABLE
...
void gpu_demo_flash_565(void *p)
{
    ...
    FILE *file = NULL;
    file = fopen("storage/res_nor_mode/C/demo/pic.bin", "r");
...
int gpu_demo_flash_565_task(void)
{
    printf(">>>>>>>>> %s()", __func__);
    int err;
    err = os_task_create(gpu_demo_flash_565, NULL, 10, 1024 * 5, 512, "gpu_demo_flash_565");
    if (err != OS_NO_ERR) {
