...
static void cube_reflection_anim2_ready_cb(struct _ui_anim_t *_anim)
{
    ...
    if (select_page == -1) {
        ...
        log_info(" map to add componenet");
    }
...
int ui_cube_reflection_ontouch(struct element_touch_event *e)
{
    ...
    case ELM_EVENT_TOUCH_UP:
        ...
        if (__this->move_flag) {
            //log_info("__this->x_curr_angle = %d; __this->y_curr_angle = %d.", __this->x_curr_angle, __this->y_curr_angle);
            //log_info("__this->pos_x = %d; __this->pos_y = %d.", __this->pos_x, __this->pos_y);
            // 移动事件之后松开触碰, 通过匀速的平移动画修正最终的模型角度
            int angle_ofs = __this->x_curr_angle % ANGLE_ANIM_SCOPE_MAX;
            if (angle_ofs != 0) {
                //log_info("angle_ofs = %d.", angle_ofs);
