...
static void hexagon_anim2_ready_cb(struct _ui_anim_t *_anim)
{
    ...
    if (select_page == -1) {
        ...
        log_debug(" map to add componenet");
    }
...
static void jlgpu_update_task_list_order(u8 *reset_sep)
{
    //log_info("last sep ===");
    //for (int i = 0; i < 6; i++) {
    //    log_info("reset_sep[%d] = %d.", i, reset_sep[i]);
    //}
...
update_list:
    ...
    if (__this->polytope_type == POLYTOPE_SIX) {
        ...
        log_error("__this->polytope_type config err\n");
        return;
    }
#endif
...
{
    int xoffset, yoffset;
    /* log_info("e->event = 0x%02x. move_flag = %d; hold_flag = %d.", e->event, __this->move_flag, __this->hold_flag); */
