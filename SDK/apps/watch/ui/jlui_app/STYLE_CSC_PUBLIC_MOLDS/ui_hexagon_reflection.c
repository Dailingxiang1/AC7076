...
#pragma bss_seg(".ui_action_hexagon_reflection.data.bss")
#pragma data_seg(".ui_action_hexagon_reflection.data")
#pragma const_seg(".ui_action_hexagon_reflection.text.const")
#pragma code_seg(".ui_action_hexagon_reflection.text")
#endif
...
#if (REF_POLYTOPE_CONFIG == REF_POLYTOPE_SIX)
const u8 ref_polytope = REF_POLYTOPE_SIX;
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_EIGHT)
const u8 ref_polytope = REF_POLYTOPE_EIGHT;
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_ALL)
const u8 ref_polytope = REF_POLYTOPE_ALL;
#endif
...
struct hexagon_priv {
    ...
    u8 move_flag;
    u8 touch_flag;
    u8 ref_polytope_type; // 多面体的数量配置，多个多面体共存时使用
    ...
    s16 pos_y;
#if (REF_POLYTOPE_CONFIG == REF_POLYTOPE_SIX)
    u32 page_id[REF_POLYTOPE_SIX];
    u8 reset_sep[REF_POLYTOPE_SIX * 2];       // 一共 12 个面
    u8 reset_sep_last[REF_POLYTOPE_SIX * 2]; // 一共 12 个面
#else //八面体或六面体和八面体共存
    u32 page_id[REF_POLYTOPE_EIGHT];
    u8 reset_sep[REF_POLYTOPE_EIGHT * 2];       // 一共 16 个面
    u8 reset_sep_last[REF_POLYTOPE_EIGHT * 2]; // 一共 16 个面
#endif
    int init : 1;
...
// 说明：以下参数是经过调整的，不要修改
#define X_ANGLE_ADJ 1.05f   // 角度 1.05f 调整后接近屏幕正面
#define X_ANGLE 15.85f

#define VIEX_DISTANCE_MIN   4.7f
#define VIEX_DISTANCE_MAX   2.595f
#define VIEX_DISTANCE_DIFF  (VIEX_DISTANCE_MIN - VIEX_DISTANCE_MAX)
#define W_SCALE     1.275f
...
void reflection_effect_init(struct element *curr_elm, bool list_flag);
void reflection_effect_uninit(void);
int ui_reflection_move(int curr_win, int xoffset, int yoffset, int mode);
int ui_reflection_zoom_out(int curr_win, int v);
int ui_reflection_zoom_in(int curr_win, int v);
int ui_page_num();
...
static void hexagon_anim0_exec_cb(int var, int32_t v)
{
    ...
    /* log_info("diff:%f, %f, v:%d \n", x_diff, y_diff, v); */

    ui_reflection_move(__this->curr_win, x_diff, y_diff, ui_card_get_move_mode());
}
...
{
    //log_info("[%s] v = %d.", __func__, v);
    ui_reflection_zoom_out(__this->curr_win, v);
}
/* ------------------------------------------------------------------------------------*/
/**
 ...
 */
/* ------------------------------------------------------------------------------------*/
static void hexagon_anim1_start()
{
    hexagon_anim1_stop();
    ...
    //log_info("curr_win : 0x%x, curr_elm : 0x%x\n", curr_win, (u32)curr_elm);
...
static void hexagon_anim2_exec_cb(int var, int32_t v)
{
    ...
    ui_reflection_zoom_in(__this->curr_win, v);
}
...
static void hexagon_anim2_ready_cb(struct _ui_anim_t *_anim)
{
    ...
    //log_info("*********************** face_index = %d; select_page = 0x%x.", __this->p_anim2->face_index, select_page);
    //log_info("__this->x_curr_angle = %d; __this->y_curr_angle = %d.", __this->x_curr_angle, __this->y_curr_angle);

    if (select_page == -1) {
        select_page = ID_WINDOW_COMPONENT;
        log_error(" map to add componenet");
    }

    reflection_effect_uninit();
...
static void hexagon_anim2_start(int face_index)
{
    ...
    int face_y_angle;
    //log_info("__this->x_curr_angle = %d; __this->y_curr_angle = %d.", __this->x_curr_angle, __this->y_curr_angle);
    reflection_get_face_angle(__this->p_anim2->face_index, &face_x_angle, &face_y_angle);
    ...
    //log_info("__this->x_curr_angle = %d; __this->y_curr_angle = %d.", __this->x_curr_angle, __this->y_curr_angle);
    //log_info("face_x_angle = %d; face_y_angle = %d.", face_x_angle, face_y_angle);
    //log_info("face_x_ofs = %d; face_y_ofs = %d.", face_x_ofs, face_y_ofs);
...
static void hexagon_anim3_exec_cb(int var, int32_t v)
{
    ...
    //log_info("__this->x_curr_angle = %d.", __this->x_curr_angle);
    //log_info("[%s] v = %d.", __func__, v);
...
static void hexagon_anim3_start(int angle_ofs, int run_time)
{
    ...
    //log_info("__this->x_curr_angle = %d, angle_ofs = %d.", __this->x_curr_angle, angle_ofs);
    //log_info("__this->x_dir = %d.", __this->x_dir);
    ...
    //log_info("time_div = %d; run_time = %d; end_dist = %d.", time_div, run_time, end_dist);
...
static void jlgpu_update_task_list_order(u8 *reset_sep)
{
    ...
    for (int i = 0; i < __this->ref_polytope_type * 2; i++) {
...
update_list:
    ...
    u8 order_tab[] = {0, reset_sep[0], reset_sep[1], reset_sep[2], reset_sep[3], reset_sep[4], reset_sep[5], \
                      reset_sep[6], reset_sep[7], reset_sep[8], reset_sep[9], reset_sep[10], reset_sep[11]
                     };
    u16 _order_tab_size = sizeof(order_tab);
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_EIGHT)
    u8 order_tab[] = {0, reset_sep[0], reset_sep[1], reset_sep[2], reset_sep[3], reset_sep[4], reset_sep[5], \
                      reset_sep[6], reset_sep[7], reset_sep[8], reset_sep[9], reset_sep[10], reset_sep[11], \
                      reset_sep[12], reset_sep[13], reset_sep[14], reset_sep[15]
                     };
    u16 _order_tab_size = sizeof(order_tab);
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_ALL)
    ...
    u8 order_tab1[] = {0, reset_sep[0], reset_sep[1], reset_sep[2], reset_sep[3], reset_sep[4], reset_sep[5], \
                       reset_sep[6], reset_sep[7], reset_sep[8], reset_sep[9], reset_sep[10], reset_sep[11], \
                       reset_sep[12], reset_sep[13], reset_sep[14], reset_sep[15]
                      };
    u8 *order_tab = NULL;
    u16 _order_tab_size = 0;
    if (__this->ref_polytope_type == REF_POLYTOPE_SIX) {
        order_tab = order_tab0;
        _order_tab_size = sizeof(order_tab0);
    } else if (__this->ref_polytope_type == REF_POLYTOPE_EIGHT) {
        order_tab = order_tab1;
        _order_tab_size = sizeof(order_tab1);
    } else {
        log_error("__this->ref_polytope_type config err\n");
        return;
    }
#endif
    int order_tab_size = _order_tab_size / sizeof(order_tab[0]);

    jlgpu_task_list_copy_group_order_adjust(__this->hexagon.head, order_tab, order_tab_size);

#if (REF_POLYTOPE_CONFIG == REF_POLYTOPE_SIX)
    u8 global_alpha_tab[1 + 6 + 6] = {32, 32, 32, 32, 32, 32, 32, 128, 128, 128, 128, 128, 128};    // 阴影部分透明处理
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_EIGHT)
    u8 global_alpha_tab[1 + 8 + 8] = {32, 32, 32, 32, 32, 32, 32, 32, 32, 128, 128, 128, 128, 128, 128, 128, 128};    // 阴影部分透明处理
#elif (REF_POLYTOPE_CONFIG == REF_POLYTOPE_ALL)
    u8 global_alpha_tab0[1 + 6 + 6] = {32, 32, 32, 32, 32, 32, 32, 128, 128, 128, 128, 128, 128};    // 阴影部分透明处理
    u8 global_alpha_tab1[1 + 8 + 8] = {32, 32, 32, 32, 32, 32, 32, 32, 32, 128, 128, 128, 128, 128, 128, 128, 128};    // 阴影部分透明处理
    u8 *global_alpha_tab = NULL;
    if (__this->ref_polytope_type == REF_POLYTOPE_SIX) {
        global_alpha_tab = global_alpha_tab0;
    } else if (__this->ref_polytope_type == REF_POLYTOPE_EIGHT) {
        global_alpha_tab = global_alpha_tab1;
    } else {
        log_error("__this->ref_polytope_type config err\n");
        return;
    }
    ...
    jlgpu_task_list_copy_group_global_alpha_reset(__this->hexagon.head, order_tab, global_alpha_tab, 1 + __this->ref_polytope_type);    // 1 + 6
...
static u8 custom_ref_polytope_type = REF_POLYTOPE_SIX;
void hexagon_custom_ref_polytope_type_set(u8 type)
{
    custom_ref_polytope_type = type;
}

/* ------------------------------------------------------------------------------------*/
/**
 * @brief reflection_effect_init 3d灯笼 + 倒影初始化
 *
 * @param curr_elm
 * @param list_flag
 */
...
void reflection_effect_init(struct element *curr_elm, bool list_flag)
{
    ...
    if (ref_polytope == REF_POLYTOPE_SIX) {
        __this->ref_polytope_type = REF_POLYTOPE_SIX;
    } else if (ref_polytope == REF_POLYTOPE_EIGHT) {
        __this->ref_polytope_type = REF_POLYTOPE_EIGHT;
    } else if (ref_polytope == REF_POLYTOPE_ALL) {
        __this->ref_polytope_type = custom_ref_polytope_type; //用户自己定义，默认6面体
        hexagon_reflection_polytope_type_set(__this->ref_polytope_type);
    } else {
        log_error("polytope config err\n");
        return;
    }
    ...
    jlgpu_task_list_copy_create(__this->hexagon.head, head, NULL, 1 + __this->ref_polytope_type);
    ...
    while (page_num--) {
        ...
        if (page_index == __this->ref_polytope_type) {
            break;
        }
    }
    for (; page_index < __this->ref_polytope_type; page_index++) {
        __this->page_id[page_index] = -1;
    }

    int win;
    struct element *win_elm;
    for (int i = 1; i < __this->ref_polytope_type; i++) {
        ...
        if (image_id != -1) {
            ...
            jlgpu_task_list_copy_create(__this->hexagon.head, new_head, NULL, i + 1 + __this->ref_polytope_type);
            jlgpu_delete_task_list_head(new_head);
        } else {
            if (win != DIAL_PAGE_0) {
                ...
                if (!win_elm) {
                    ...
                    if (!ret) {
                        ...
                        jlgpu_task_list_copy_create(__this->hexagon.head, win_elm->dc->gpu_task_head, NULL, i + 1 + __this->ref_polytope_type);
                        ui_hide(win);
                    }
                } else {
                    jlgpu_task_list_copy_create(__this->hexagon.head, win_elm->dc->gpu_task_head, NULL, i + 1);
    ...
    reflection_init(__this->width_tab, __this->height_tab, ((float)win_rect.height / (float)win_rect.width) * W_SCALE, H_SCALE, VIEX_DISTANCE_MIN, win_rect.left, win_rect.top, win_rect.width, win_rect.height, (X_ANGLE - X_ANGLE_ADJ));
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief reflection_effect_update 特效更新，用于刷新时间
 *
 * @param timeout
 */
/* ------------------------------------------------------------------------------------*/
void reflection_effect_update(int timeout)
{
    int i;
    ...
    for (i = 0; i < __this->ref_polytope_type; i++) {
    ...
    if (dial_page_index != -1) {
        ...
        if (win_elm) {
            ...
            ret = jlgpu_task_list_copy_update_matrix_by_group(__this->hexagon.head, win_elm->dc->gpu_task_head, dial_page_index + 1 + 6);
            if (ret) {
                jlgpu_task_list_copy_destroy_by_group(__this->hexagon.head, dial_page_index + 1 + 6);
                ASSERT(win_elm->dc);
                jlgpu_task_list_copy_create(__this->hexagon.head, win_elm->dc->gpu_task_head, NULL, dial_page_index + 1 + 6);
            }
        }

        // 更新模型的显示, 播放动画时不更新
        if ((!hexagon_anim1_is_playing()) && (!hexagon_anim2_is_playing())) {
            reflection_draw(&__this->hexagon, __this->x_curr_angle, __this->y_curr_angle, X_ANGLE_ADJ, 0.0f, __this->reset_sep, 0);  // 这个 draw 不会真正绘制，只是为了更新 list
            jlgpu_update_task_list_order(__this->reset_sep); // 这里根据页面绘制顺序更新链表
            reflection_draw(&__this->hexagon, __this->x_curr_angle, __this->y_curr_angle, X_ANGLE, (X_ANGLE - X_ANGLE_ADJ), __this->reset_sep, 1);
        }
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief reflection_effect_uninit 反初始化
 */
/* ------------------------------------------------------------------------------------*/
void reflection_effect_uninit(void)
{
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief ui_reflection_status 特效执行状态
 *
 * @return
 */
/* ------------------------------------------------------------------------------------*/
...
int ui_reflection_move(int curr_win, int xoffset, int yoffset, int mode)
{
    ...
    if (!curr_elm) {
        log_error("curr_win : 0x%x, curr_elm : 0x%x\n", curr_win, (u32)curr_elm);
    ...
    __this->cur_left = (__this->cur_left >= page_width) ? page_width : __this->cur_left;
    __this->cur_left = (__this->cur_left <= -page_width) ? -page_width : __this->cur_left;

    if ((__this->cur_left != 0) && (__this->init == 0)) {
        reflection_effect_init(curr_elm, false);
    }
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief ui_reflection_zoom_out 入场动画: 缩小
 *
 * @param curr_win
 * @param v
 *
...
int ui_reflection_zoom_out(int curr_win, int v)
{
    ...
    lcdrect.height = page_height;
#if 0
    if ((__this->init == 0) && (__this->hold_flag == 1)) {
        reflection_effect_init(curr_elm, false);
    }
#else
    if (__this->init == 0) {
...
#endif
    reflection_uninit();
    reflection_init(__this->width_tab, __this->height_tab, ((float)win_rect.height / (float)win_rect.width) * W_SCALE, H_SCALE, (VIEX_DISTANCE_MAX + ((VIEX_DISTANCE_DIFF * v) / SCALE_ANIM_SCOPE)), win_rect.left, win_rect.top, win_rect.width, win_rect.height, (X_ANGLE - X_ANGLE_ADJ));
    ...
    if (__this->init) {
        ...
        reflection_draw(&__this->hexagon, __this->x_curr_angle, __this->y_curr_angle, (x_angle_val + X_ANGLE_ADJ), x_angle_val, __this->reset_sep, 1);
    }
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief ui_reflection_zoom_in 离场动画: 放大
 *
 * @param curr_win
 * @param v
 *
...
int ui_reflection_zoom_in(int curr_win, int v)
{
    ...
    if (__this->init == 0) {
        log_error("%s error!! __this->init == 0.\n", __func__);
        return -1;
    }
...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief ui_reflection_ontouch 触摸
 *
 * @param e
 *
 * @return
...
{
    int xoffset, yoffset;
    /* log_info("%s e->event = 0x%02x. move_flag = %d; hold_flag = %d.",__func__, e->event, __this->move_flag, __this->hold_flag); */
    if (hexagon_anim1_is_playing() || hexagon_anim2_is_playing()) { //入场和离场动画不允许打断
        return true;
    }
    ...
    case ELM_EVENT_TOUCH_MOVE:
        ...
        __this->pos_x = e->pos.x;
        __this->pos_y = e->pos.y;
        //log_info("id = 0x%x.", ui_get_current_window_id());
        if (__this && __this->init) {
            ui_reflection_move(ui_get_current_window_id(), xoffset, yoffset, ui_card_get_move_mode());
        }
        __this->move_flag = 1;
#else
    ...
    case ELM_EVENT_TOUCH_UP:
        ...
        int face_index = reflection_get_face_index(e->pos.x, e->pos.y);
        //log_info("touch face %d\n", face_index);
        if (face_index != -1) {
...
REGISTER_UI_EFFECT_MODULE(PAGE_MOVE_MODE_REFLECTION)
.ontouch = ui_reflection_ontouch,
 .get_status = ui_reflection_status,
  .uninit = reflection_effect_uninit,
};
#endif
