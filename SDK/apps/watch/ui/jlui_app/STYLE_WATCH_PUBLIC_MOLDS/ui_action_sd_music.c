    ...
    case CTRL_TYPE_TEXT:
        ...
        if (!strcmp(text->source, "name")) {
            ...
            if (info->fn_type) {
                ...
                text->attrs.encode = FONT_ENCODE_UTF8;//FONT_ENCODE_ANSI;
                text->attrs.str    = info->lfn_buf.lfn;
                /* put_buf((u8 *)info->lfn_buf.lfn, 512); */
...
static int file_select_enter(u32 index)
{
    ...
    if (info->dir_type == BS_DIR_TYPE_FORLDER) {
        ...
        ui_text_set_textu_by_id(MUSIC_LYRICS_TEXT, music_handler->name, strlen(music_handler->name), FONT_DEFAULT);
        return 0;
    }
...
static int music_name_enter_onchane(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_FIRST_SHOW:
        if (music_handler) {
            ...
            ui_text_set_text_attrs(text, music_handler->name, strlen(music_handler->name), FONT_ENCODE_UTF8, 0, FONT_DEFAULT | FONT_SHOW_SCROLL);
        }
        break;
    case ON_CHANGE_SHOW:
...
REGISTER_UI_EVENT_HANDLER(MUSIC_LYRICS_TEXT)
.onchange = music_name_enter_onchane,
 .onkey = NULL,
  .ontouch = NULL,
};
