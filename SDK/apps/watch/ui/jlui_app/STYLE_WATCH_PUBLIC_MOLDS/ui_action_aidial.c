...
static int reflash_txt_handler();
static int reflash_getdialStyle_handler(int a);
static void AI_DIAL_WAVE_handler();
static void page_show();
static void ai_normal(void);
static int show_aidial();
static int change_error();
...
static u8 cuntdown_flag = 60; //60s倒计时
volatile static u8 dial_button = 0; //开始录音的按钮控件取反标志位 0----开始录音  1----停止录音
...
void put_aidial(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (op == 0X00) {
        u8 *temp = (u8 *)(data + 2);
        size_t bylen = strlen((char *)temp);
        size_t hlen = bylen / sizeof(u8);
        *(temp + hlen - 1) = '\0';
        put_buf(temp, hlen - 1);
        if (aidial_txt) {
            free((void *)aidial_txt);
            aidial_txt = NULL;
        }
        aidial_txt = (u8 *)strdup((char *)temp);
        ui_log("ZEZEZEstring:%s\n", aidial_txt);
        put_buf((const u8 *)aidial_txt, hlen);
        JL_CMD_response_send(OpCode, JL_PRO_STATUS_SUCCESS, OpCode_SN, (u8 *)&ai_responce_msg, sizeof(struct AI_RESPONCE_MSG), 0, NULL);
    }
    if (op == 0X04) {
        ui_log("FOCUS!!!\n");
        JL_CMD_response_send(OpCode, JL_PRO_STATUS_SUCCESS, OpCode_SN, (u8 *)&ai_responce_msg, sizeof(struct AI_RESPONCE_MSG), 0, NULL);
...
static void send_dialWithoutState_cmd(u8 status, u8 flag_type)
{
    ...
    JL_CMD_send(JL_OPCODE_AI_DIAL, (u8 *)&ai_dial_sen, sizeof(struct AI_DIAL_SEN), JL_NEED_RESPOND, 0, NULL);
}
...
static void send_aidial_cmd_normal(u8 status, u8 flag_type, u8 state)
{
    ...
    JL_CMD_send(JL_OPCODE_AI_DIAL, (u8 *)&ai_dial_send, sizeof(struct AI_DIAL_MSG_INFO), JL_NEED_RESPOND, 0, NULL);
...
static void send_aidial_cmd_expand(u8 status, u8 flag_type, u8 state)
{
    ...
    put_buf((const u8 *)&ai_dial_exp, sizeof(struct AI_DIAL_EXP_INFO));
    JL_CMD_send(JL_OPCODE_AI_DIAL, (u8 *)&ai_dial_exp, sizeof(struct AI_DIAL_EXP_INFO), JL_NEED_RESPOND, 0, NULL);
}
...
static void page_switch(u8 ui_temp)
{
    ...
    if ((get_rcsp_connect_status()) && (call_ctrl_get_status() != BT_CALL_ACTIVE)) {
        aidial_core = ui_temp;
...
static int reflash_getdialStyle_handler(int a)
{
    ...
    if (aidial_txt) {
        ui_text_set_textu_by_id(AI_DIAL_STYLE, (char *)aidial_txt, strlen((char *)aidial_txt), FONT_DEFAULT);
    }
    return 0;
}
...
{
    if (ai_dial_txt1) {
        memset((char *)ai_dial_txt1, '\0', strlen((char *)ai_dial_txt1) + 1);
    }
}

static void ai_txt_empty_handler(void *priv)
{
    if (empty_str && (!ai_dial_txt1 || strlen((char *)ai_dial_txt1) == 0)) {
...
//								layout0-AI表盘											//
/****************************************************************************************/
static int change_text(int a)
{
    ui_text_show_index_by_id(AI_DIAL_STATUS, 0);
    return 0;
}
static int change_phone_txt(int a)
{
...
static int AI_dial_txt_onchange(void *ctr, enum element_change_event e, void *arg)
{
    switch (e) {
    case ON_CHANGE_INIT:
        if (!empty_str) {
    ...
    case ON_CHANGE_SHOW_POST:
        if (!txt_timer) {
            txt_timer = sys_timeout_add(NULL, ai_txt_empty_handler, 2000);
        }
        break;
    case ON_CHANGE_RELEASE:
        if (txt_timer) {
...
REGISTER_UI_EVENT_HANDLER(AI_DIAL_TXT)
.onchange = AI_dial_txt_onchange,
 .onkey = NULL,
  .ontouch = NULL,
};
//****************************************************************************************/
...
static void AI_thinking_handler(void *priv)
{
    ...
    static int i = 0;
    i++;
    struct ui_pic *pic = ui_pic_for_id(AI_DIAL_THINKING);
    if (pic) {
        i++;
...
static int AI_DIAL_PICII_onchange(void *ctr, enum element_change_event e, void *arg)
{
    ...
    switch (e) {
    case ON_CHANGE_INIT:
        ai_photo.file = (RESFILE *)res_fopen(aidial_bgp_path, "r");
        if (!ai_photo.file) {
            return FALSE;
        }
        ui_res_flash_info_get(&ai_photo.file_info, aidial_bgp_path, "res", 1);
        break;
    case ON_CHANGE_SHOW:
        if (ai_photo.file) {
            pic->elm.css.background_image = 1;
            dc->preview.file = (UI_RESFILE *)ai_photo.file;
    ...
    case ON_CHANGE_RELEASE:
        if (ai_photo.file) {
            res_fclose((UI_RESFILE *)ai_photo.file);
