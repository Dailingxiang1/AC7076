...
#define LOG_TAG     		"[UI-OUTDOORSPORTS]"
...
#pragma bss_seg(".ui_action_outdoorsports.data.bss")
#pragma data_seg(".ui_action_outdoorsports.data")
#pragma const_seg(".ui_action_outdoorsports.text.const")
#pragma code_seg(".ui_action_outdoorsports.text")
#endif


#ifdef CONFIG_UI_STYLE_JL_PUBLIC_MODLS_ENABLE
#if TCFG_UI_ENABLE_OUTDOORSPORTS
...
static int outdoor_sports_init(int p)
{
    struct element *elm;
    elm = ui_core_get_element_by_id(OUTDOOR_SPORT_LIST);
...
static int outdoor_vlist_default_onchange(void *_ctrl, enum element_change_event event, void *arg)
{
...
static int outdoor_sports_layout_onchange(void *ctr, enum element_change_event e, void *arg)
{
    int center, ratio;
    ...
    case ON_CHANGE_FIRST_SHOW:
        ui_set_call(outdoor_sports_init, 0);
        break;
...
REGISTER_UI_EVENT_HANDLER(OUTDOOR_SPORT_LAYOUT)
.onchange = outdoor_sports_layout_onchange,
 .onkey    = NULL,
  .ontouch  = NULL,
};


static int outdoor_sports_list_ontouch(void *ctr, struct element_touch_event *e)
{
    u8 idx = 0;
    ...
    case ELM_EVENT_TOUCH_UP:
        ...
        if (__this->target) {
            ...
            if (e->pos.x >= child_rect.left && e->pos.x <= (child_rect.left + child_rect.width) && \
                ...
                sporting_target_record(ID_WINDOW_OUTDOOR_SPORTS);
                ui_hide_curr_main();
                ui_show_main(ID_WINDOW_SPORT_TARGET);
                UI_WINDOW_BACK_DEL(ID_WINDOW_OUTDOOR_SPORTS);
                __this->target = NULL;
                break;
            }
        }
        int sport_type  = sport_type_map_get_type(SPORT_MODE_OUTDOOR, __this->index);
...
REGISTER_UI_EVENT_HANDLER(OUTDOOR_SPORT_LIST)
.onchange = outdoor_vlist_default_onchange,
 .onkey    = NULL,
  .ontouch  = outdoor_sports_list_ontouch,
};


static int outdoor_sports_page_onchange(void *ctr, enum element_change_event e, void *arg)
{
    switch (e) {
    case ON_CHANGE_INIT:
        if (!__this) {
...
static int outdoor_sports_page_ontouch(void *elm, struct element_touch_event *e)
{
    switch (e->event) {
...
REGISTER_UI_EVENT_HANDLER(ID_WINDOW_OUTDOOR_SPORTS)
.onchange = outdoor_sports_page_onchange,
 .onkey    = NULL,
  .ontouch  = NULL,
};


#endif/*#if TCFG_UI_ENABLE_OUTDOORSPORTS*/
