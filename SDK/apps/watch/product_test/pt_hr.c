...
#if (PT_HR_ENABLE &&TCFG_SENSOR_HUB)
/*****************************************************/
//				PT_SENSOR_CONFIG
/*****************************************************/
...
static int pt_hr_data_get(void *data_buf, int data_len, int mode)
{
    if (PT_HR_MODE_HEART_RATE == mode) {
        return sensor_hub_get_data(SENSOR_DRV_HR, 0, data_buf, data_len);
    } else if (PT_HR_MODE_SPO2 == mode) {
        return sensor_hub_get_data(SENSOR_DRV_SPO2, 0, data_buf, data_len);
    }
    return 0;
}
//***********************************************************//
...
static int pt_hr_hrmode_test(void)
{
    ...
    sensor_hub_enable(SENSOR_DRV_HR, 1);
    sensor_hub_cbuf_enable(SENSOR_DRV_HR, 0, 1);
    os_time_dly(6);
    // 获取hr数据
    ...
    sensor_hub_enable(SENSOR_DRV_HR, 0);
    sensor_hub_cbuf_enable(SENSOR_DRV_HR, 0, 0);
    if (sensor_buf) {
        free(sensor_buf);
        sensor_buf = NULL;
    }
...
static int pt_hr_spo2mode_test(void)
{
    ...
    sensor_hub_enable(SENSOR_DRV_SPO2, 1);
    sensor_hub_cbuf_enable(SENSOR_DRV_SPO2, 0, 1);
    os_time_dly(6);
    // 获取hr数据
    ...
    sensor_hub_enable(SENSOR_DRV_SPO2, 0);
    sensor_hub_cbuf_enable(SENSOR_DRV_SPO2, 0, 0);
    if (sensor_buf) {
        free(sensor_buf);
        sensor_buf = NULL;
    }
...
int pt_hr_init(void)
{
    ...
    if (!info) { //设备是否注册
        pt_log_e("pt_hrsensor_dev_not_find\n");
        return PT_E_NO_DEV;
    }
    if (info->state == SENSOR_STATE_OFFLINE) { //设备是否在线
        pt_log_e("pt_hrsensor_state_error\n");
        return PT_E_DEV_ID;
    }
