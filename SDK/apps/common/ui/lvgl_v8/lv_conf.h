...
#define LV_DISPLAY_DIRECT_MODE  0

/*Always make the whole screen redrawn*/
#define LV_DISPLAY_FULL_REFRESH 0
 ...
 * GPU
 *-----------*/
#define LV_USE_GPU2D_JL     1

//GPU链表绘制
#define LV_USE_GPU_LIST_DRAW  0
#define LV_USE_GPU_LIST_TEMP  0//优化阶段使用
#define LV_USE_GPU_LIST_LETTER_BREAK	0//断点绘制字符

#define LV_TCFG_LCD_TE_USED_PEND    0

// 打印rets地址，临时调试用，后续会删掉，调试如果调用了，上传时必须删除！！！
#define JLCPU_TRACE_UP() \
{ \
    u32 rets; \
    __asm__ volatile("%0 = rets":"=r"(rets)); \
	printf("@@@@@### %s(), rets: 0x%x\n", __func__, rets); \
}
// 打印函数及当前行数，临时调试用，后续会删掉，调试如果调用了，上传时必须删除！！！
#define JLCPU_DUMP_POS() printf("--> @@@ %s(), %d\n", __func__, __LINE__);
...
#define LV_FONT_MONTSERRAT_JL 0
...
#define LV_FONT_DEFAULT &lv_font_montserrat_none

/*使用 lv_set_scale_base_font(const lv_font_t *font_p)设置缩放基准字号字体;
 *基准字体需要生成完整信息;
 *其他字体可不生成bitmap;
 *其他字体由基准字号字体缩放显示;*/
#define LV_USE_FONT_SCALE  1
...
// 整行 MCU 块进行绘制
#define LV_USE_JLJPEG_LINE_DRAW_ENABLE 1
...
/*API for jl FATFS (needs to be added separately). Uses f_open, f_read, etc*/
#define LV_USE_JL_FAT 1
...
#define LV_USE_VIDEO_TO_IMAGES 0
#if LV_USE_VIDEO_TO_IMAGES
