                                     ...
                                     lv_text_flag_t flag, uint32_t *word_w_ptr, lv_text_cmd_state_t *cmd_state, bool force)
{
    ...
    while (txt[i] != '\0') {
#if LV_FONT_MONTSERRAT_JL
        uint32_t letter_prev;
        uint32_t tmp_i = i_next_next - utf8_cnt; //记录原流程索引位置适配原流程
        _lv_txt_encoded_letter_prev(txt, &letter_prev, &tmp_i); //获取prev_text
...
#if LV_FONT_MONTSERRAT_JL
        lv_font_t jl_font;
        memcpy(&jl_font, font, sizeof(lv_font_t));
        jl_font.user_data = (void *)letter_prev;
        letter_w = lv_font_get_glyph_width((const lv_font_t *)(&jl_font), letter, letter_next);
#else
                            ...
                            lv_text_flag_t flag)
{
    ...
    if (length != 0) {
        while (i < length) {
...
#if LV_FONT_MONTSERRAT_JL
            uint32_t letter_prev;
            uint32_t tmp_i = i; //记录当前索引用来获取prev_text
            _lv_txt_encoded_letter_prev(txt, &letter_prev, &tmp_i); //获取prev_text
            ...
            memcpy(&jl_font, font, sizeof(lv_font_t));
            jl_font.user_data = (void *)letter_prev;
            lv_coord_t char_width = lv_font_get_glyph_width((const lv_font_t *)(&jl_font), letter, letter_next);
#else
...
void _lv_txt_encoded_letter_prev(const char *txt, uint32_t *letter_prev, uint32_t *ofs)
{
    *letter_prev = *ofs > 0 ? _lv_txt_encoded_prev(txt, ofs) : 0;
}
