        ...
        const lv_area_t *coords, const char *txt, lv_draw_label_hint_t *hint)
{
    ...
    while (txt[line_start] != '\0') {
        ...
        while (i < line_end - line_start) {
            ...
            uint32_t tmp_i = i; //记录原流程索引位置适配原流程
            uint32_t letter_prev;
            _lv_txt_encoded_letter_prev(bidi_txt, &letter_prev, &tmp_i); //获取prev_text
            _lv_txt_encoded_letter_next_2(bidi_txt, &letter, &letter_next, &i);
            draw_ctx->letter_prev = letter_prev;
            draw_ctx->letter_next = letter_next;
#else
            _lv_txt_encoded_letter_next_2(bidi_txt, &letter, &letter_next, &i);
#endif
