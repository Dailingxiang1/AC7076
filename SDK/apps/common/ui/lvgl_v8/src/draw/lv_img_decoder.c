 ...
 *  STATIC VARIABLES
 **********************/
#if ((LV_COMPRESS_C1 || LV_COMPRESS_C2) && !LV_USE_GPU2D_JL)
...
#if ((LV_COMPRESS_C1 || LV_COMPRESS_C2) && !LV_USE_GPU2D_JL)
        if (dsc->img_data != NULL && (compress_type == LV_COMPRESS_C1 || compress_type == LV_COMPRESS_C2)) {
            //对应驱动的dma_malloc
            dma_free(dsc->img_data);
        }
#endif
...
lv_res_t lv_img_decoder_built_in_info(lv_img_decoder_t *decoder, const void *src, lv_img_header_t *header)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        lv_img_dsc_t src_dsc;
        lv_get_img_dsc_bin_info(src, &src_dsc);
