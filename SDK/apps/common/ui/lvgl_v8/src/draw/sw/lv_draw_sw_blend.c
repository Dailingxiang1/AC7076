...
void lv_draw_sw_blend(lv_draw_ctx_t *draw_ctx, const lv_draw_sw_blend_dsc_t *dsc)
{
    ...
    if (draw_ctx->final_draw_area == NULL) {
        if (!_lv_area_intersect(&blend_area, dsc->blend_area, draw_ctx->clip_area)) {
            return;    /*Fully clipped, nothing to do*/
        }
    } else {
        if (!_lv_area_intersect(&blend_area, draw_ctx->final_draw_area, draw_ctx->clip_area)) {
            return;    /*Fully clipped, nothing to do*/
        }
    }
#else
...
#if (LV_USE_GPU2D_JL == 0||LV_USE_GPU_LIST_TEMP) //gpu2d已经适配了blend normal情况,软件blend可以不用
        if (dsc->src_buf == NULL) {
