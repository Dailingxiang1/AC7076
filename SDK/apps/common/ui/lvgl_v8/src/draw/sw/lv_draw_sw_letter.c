...
{
    scale_base_font = (lv_font_t *)font_p;
}
...
static void lv_jlgpu_zoom_a8(const uint8_t *src, int src_width, int src_height,
                             uint8_t *dst, int dst_width, int dst_height, int bpp)
{
    ...
    out_param.data = (uint8_t *)dst;
    out_param.win_x_min = 0;
    out_param.win_x_max = dst_width;
    out_param.win_y_min = 0;
    out_param.win_y_max = dst_height;

    switch (bpp) {
    case 1:
        out_param.format = GPU_FORMAT_A1;
        texture_param.format = GPU_FORMAT_A1;
        break;
    case 2:
        out_param.format = GPU_FORMAT_A2;
        texture_param.format = GPU_FORMAT_A2;
        break;
    case 4:
        out_param.format = GPU_FORMAT_A4;
        texture_param.format = GPU_FORMAT_A4;
        break;
    case 8:
        out_param.format = GPU_FORMAT_A8;
    ...
    out_param.stride = (dst_width * gpu_get_format_bpp(texture_param.format)) >> 3;

    basic_param.global_alpha = 128;
    basic_param.act_x_min = 0;
    basic_param.act_x_max = dst_width;
    basic_param.act_y_min = 0;
    basic_param.act_y_max = dst_height;

    basic_param.blend_mode = GPU_BLEND_SRC;
    basic_param.premult = 0;
    basic_param.layer_en = 1;
    basic_param.dither_en = 1;

    texture_param.data = (uint8_t *)src;
    texture_param.compress_mode = 0;
    texture_param.compress_size = (src_width * gpu_get_format_bpp(texture_param.format)) >> 3;

    texture_param.stride = (src_width * gpu_get_format_bpp(texture_param.format)) >> 3;
    ...
    float dst_width_f = (float)dst_width;
    float src_width_f = (float)src_width;
    ...
    dst_src_zoom = (dst_width_f / src_width_f) * 256;

    zoom = LV_IMG_ZOOM_NONE / (dst_src_zoom * 1.0f);
    ...
    transform_param.fg_x_max = src_width;
    transform_param.fg_y_min = 0;
    transform_param.fg_y_max = src_height;
    ...
    if (0) {
        //TODO 抗锯齿待优化
        transform_param.sample_mode = 3;
        transform_param.shift_sel = 5;
        transform_param.coeff0 = 3;
        transform_param.coeff1 = 3;
        transform_param.coeff2 = 8;
        basic_param.blend_mode = GPU_BLEND_SRC;
        basic_param.ext_mode = 1; //硬件自动扩充像素
    }
...
#if LV_USE_GPU_LIST_DRAW&&(!LV_USE_GPU_LIST_TEMP)
    //输出gpu链表
    lv_gpu_task_list_run_break(draw_ctx);
    draw_ctx->final_draw_area = NULL;   // 不涉及 GPU list 的最终绘制位置
    draw_ctx->attachments = NULL;
#endif

    lv_font_glyph_dsc_t g;
#if LV_FONT_MONTSERRAT_JL
    lv_font_t jl_font;
    memcpy(&jl_font, dsc->font, sizeof(lv_font_t));
    jl_font.user_data = (void *)(uint32_t)draw_ctx->letter_prev;
    bool g_ret = lv_font_get_glyph_dsc((const lv_font_t *)(&jl_font), &g, letter, draw_ctx->letter_next);
#if LV_USE_FONT_SCALE
    lv_font_glyph_dsc_t g_base;
    if (scale_base_font != dsc->font) {
        if (scale_base_font) {
            scale_base_font->user_data = (void *)(uint32_t)draw_ctx->letter_prev;
            g_ret = lv_font_get_glyph_dsc(scale_base_font, &g_base, letter, draw_ctx->letter_next);
        } else {
            LV_LOG_ERROR("lv_draw_letter: not set scale_base_font!\n");
            g_ret = lv_font_get_glyph_dsc((const lv_font_t *)(&jl_font), &g_base, letter, draw_ctx->letter_next);
        }
    }
#endif
#else
...
#if LV_USE_GPU2D_JL
                //gpu硬件缩放
                lv_jlgpu_zoom_a8(map_p, g_base.box_w, g_base.box_h, zoom_buf, g.box_w, g.box_h, g.bpp);
#else
