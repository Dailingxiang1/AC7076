                            ...
                            const lv_draw_img_dsc_t *draw_dsc)
{
    ...
    uint8_t malloc_fail = 0;
    //保存src_buf数据
    if (layer_sw_ctx->has_alpha) {
        img.data = lv_gpu_attach_buf_add_node(draw_ctx, draw_ctx->buf, img.header.w * img.header.h * 3);
    } else {
        img.data = lv_gpu_attach_buf_add_node(draw_ctx, draw_ctx->buf, img.header.w * img.header.h * 2);
    }
    if (img.data == NULL) {
        LV_LOG_ERROR("malloc img.data error!!!");
        malloc_fail = 1;
