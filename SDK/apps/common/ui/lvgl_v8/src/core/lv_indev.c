...
static void indev_proc_release(_lv_indev_proc_t *proc)
{
    ...
    if (indev_obj_act) {
        ...
        if (scroll_obj == NULL) {
            ...
            /* proc->longpr_rep_timestamp: 长按事件标志 */
            /* lv_tick_get() - proc->pr_timestamp: 当前时间戳和按下时间戳的差值ms */
            uint8_t abs_x = LV_ABS(proc->types.pointer.act_point.x - proc->types.pointer.last_point.x); //按下到抬起的横坐标差值pixel
            uint8_t abs_y = LV_ABS(proc->types.pointer.act_point.y - proc->types.pointer.last_point.y); //按下到抬起的纵坐标差值pixel
            if (!proc->longpr_rep_timestamp && lv_tick_get() - proc->pr_timestamp < 150 && abs_x < 5 && abs_y < 5) {
                lv_event_send(indev_obj_act, LV_EVENT_CLICKED, indev_act);
            }
            if (indev_reset_check(proc)) {
                return;
