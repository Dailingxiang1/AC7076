...
void lv_obj_redraw(lv_draw_ctx_t *draw_ctx, lv_obj_t *obj)
{
    ...
    draw_ctx->compress_type = 0;
    draw_ctx->res_block = LV_RAW_BLOCK_NONE;
...
void _lv_disp_refr_timer(lv_timer_t *tmr)
{
    ...
    if (disp_refr->bg_scr) {
        lv_obj_update_layout(disp_refr->bg_scr);
    }
...
static void refr_invalid_areas(void)
{
    ...
    for (i = 0; i < disp_refr->inv_p; i++) {
        ...
        if (disp_refr->inv_area_joined[i] == 0) {
            ...
            disp_refr->driver->draw_buf->last_part = 0;
            disp_refr->cur_refr_area = i; //推屏需要用到,用于记录当前刷新到第几个脏矩形
...
#if LV_DISPLAY_FULL_REFRESH || LV_DISPLAY_DIRECT_MODE
...
static void refr_area_part(lv_draw_ctx_t *draw_ctx)
{
    ...
    lv_gpu_task_list_create(draw_ctx, GPU_OUT_FORMAT_RGB565);
    ...
    if (disp_refr->bg_scr) {
        top_bg_scr = lv_refr_get_top_obj(draw_ctx->buf_area, disp_refr->bg_scr);
    }
    ...
    if (disp_refr->draw_prev_over_act) {
        ...
        if (disp_refr->bg_scr) {
            if (top_bg_scr == NULL) {
                top_bg_scr = disp_refr->bg_scr;
            }
            refr_obj_and_children(draw_ctx, top_bg_scr);
        }
...
#if LV_USE_GPU2D_JL //硬件GPU可以方便做到分块变换+抗锯齿
...
static void draw_buf_flush(lv_disp_t *disp)
{
    ...
    /* extern void lv_gpu_task_list_blend(lv_disp_t *disp); */
    lv_gpu_task_list_blend(disp);
    return;
