...
#if LV_FONT_MONTSERRAT_JL
    return lv_font_letter_bitmap_get((u16)unicode_letter);
#endif
...
#if LV_FONT_MONTSERRAT_JL
    u16 unicode_letter_prev = (u16)font->user_data;
    short unicinfo[5];
    lv_font_letter_info_get(unicode_letter, unicode_letter_next, unicode_letter_prev, unicinfo);
    if (unicinfo[0] == 0 || unicinfo[1] == 0) {
        return false;
    }
    dsc_out->adv_w = unicinfo[4];
    dsc_out->box_h = unicinfo[1];
    dsc_out->box_w = unicinfo[0];
    dsc_out->ofs_x = unicinfo[2];
    dsc_out->ofs_y = unicinfo[3] - unicinfo[1];
    dsc_out->bpp   = lv_font_get_bpp();
