...
void lv_img_set_src(lv_obj_t *obj, const void *src)
{
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
    ...
    } else if (src_type == LV_IMG_SRC_BIN && lv_get_jl_src_variable(src)) {
        img->src = src;
        ...
        if (img->src != src) {
            ...
            if (src_type == LV_IMG_SRC_BIN && lv_pack_mode == LV_PACK_MODE_TIGHT) {
                new_str = lv_mem_alloc(sizeof(uint32_t));
            } else {
                new_str = lv_mem_alloc(strlen(src) + 1);
            }
            ...
            if (src_type == LV_IMG_SRC_BIN && lv_pack_mode == LV_PACK_MODE_TIGHT) {
                lv_memcpy(new_str, src, sizeof(uint32_t));
            } else {
                strcpy(new_str, src);
            }
...
static void lv_img_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    lv_img_t *img = (lv_img_t *)obj;
    if (img->src_type == LV_IMG_SRC_FILE || img->src_type == LV_IMG_SRC_SYMBOL || img->src_type == LV_IMG_SRC_BIN) {
        if (!lv_get_jl_src_variable((uint8_t *)img->src)) {
            lv_mem_free((void *)img->src);
        }
        img->src      = NULL;
        img->src_type = LV_IMG_SRC_UNKNOWN;
