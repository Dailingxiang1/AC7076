...
#include "extra/libs/binfont/lv_font_unic.h"
#endif
...
void lv_label_set_text(lv_obj_t *obj, const char *text)
{
    ...
    if (label->text == text && label->static_txt == 0) {
        ...
        u16 *utf16 = lv_mem_alloc(strlen(text) * 2);
        lv_memset_00(utf16, strlen(text) * 2);
        int utf16_len = lv_utf8_to_utf16((u8 *)text, strlen(text), utf16, 0);
        u16 update_strlen = lv_font_mixrightword_update((u8 *)utf16, utf16_len);
        u8 *utf8 = lv_mem_alloc(update_strlen * 2);
        int utf8_len = lv_utf16_to_utf8((u8 *)utf16, update_strlen, utf8, 0);
        lv_mem_free(utf16);
        /*Allocate space for the new text*/
        label->text = lv_mem_alloc(utf8_len + 1);

        LV_ASSERT_MALLOC(label->text);
        if (label->text == NULL) {
            return;
        }
        label->text[utf8_len] = 0;
        memcpy(label->text, utf8, utf8_len);
        lv_mem_free(utf8);
