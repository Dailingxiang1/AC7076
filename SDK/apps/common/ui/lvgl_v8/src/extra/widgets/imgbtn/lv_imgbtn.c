                       ...
                       const void *src_right)
{
    ...
    lv_imgbtn_t *imgbtn = (lv_imgbtn_t *)obj;
    uint8_t lv_pack_mode = lv_get_ui_pack_mode();
    ...
    if (src_left) {
        src_type = lv_img_src_get_type(src_left);
        if (src_type == LV_IMG_SRC_BIN) {
            if (lv_pack_mode == LV_PACK_MODE_TIGHT) {
                src_left_str = lv_mem_alloc(sizeof(uint32_t));
                LV_ASSERT_MALLOC(src_left_str);
                lv_memcpy(src_left_str, src_left, sizeof(uint32_t));
            } else {
    ...
    if (src_mid) {
        src_type = lv_img_src_get_type(src_mid);
        if (src_type == LV_IMG_SRC_BIN) {
            if (lv_pack_mode == LV_PACK_MODE_TIGHT) {
                src_mid_str = lv_mem_alloc(sizeof(uint32_t));
                LV_ASSERT_MALLOC(src_mid_str);
                lv_memcpy(src_mid_str, src_mid, sizeof(uint32_t));
            } else {
    ...
    if (src_right) {
        src_type = lv_img_src_get_type(src_right);
        if (src_type == LV_IMG_SRC_BIN) {
            if (lv_pack_mode == LV_PACK_MODE_TIGHT) {
                src_right_str = lv_mem_alloc(sizeof(uint32_t));
                LV_ASSERT_MALLOC(src_right_str);
                lv_memcpy(src_right_str, src_right, sizeof(uint32_t));
            } else {
