...
static void release_indicator_src(lv_analogclock_indicator_t *indic);
static void release_dial_src(lv_analogclock_dial_t *dial);
static void set_needle_img(lv_analogclock_indicator_t **indic_ptr, lv_obj_t *obj, const void *src,
...
{
    lv_analogclock_t *analogclock = (lv_analogclock_t *)obj;
    set_needle_img(&analogclock->hour_indic, obj, src, pivot_x, pivot_y);
}
...
{
    lv_analogclock_t *analogclock = (lv_analogclock_t *)obj;
    set_needle_img(&analogclock->min_indic, obj, src, pivot_x, pivot_y);
}
...
{
    lv_analogclock_t *analogclock = (lv_analogclock_t *)obj;
    set_needle_img(&analogclock->sec_indic, obj, src, pivot_x, pivot_y);
}
        ...
        lv_coord_t pivot_x, lv_coord_t pivot_y)
{
    ...
    uint8_t src_type = lv_img_src_get_type(src);
    if (src_type == LV_IMG_SRC_VARIABLE) {
        indic->type_data.needle_img.src = src;
        indic->type_data.needle_img.src_type = src_type;
    } else if (src_type == LV_IMG_SRC_SYMBOL || src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_BIN) {
        char *new_str = lv_mem_alloc(strlen(src) + 1);
        LV_ASSERT_MALLOC(new_str);
        if (new_str) {
            strcpy(new_str, src);
            indic->type_data.needle_img.src = new_str;
            indic->type_data.needle_img.src_type = src_type;
        }
    } else {
        indic->type_data.needle_img.src = NULL;
        indic->type_data.needle_img.src_type = LV_IMG_SRC_UNKNOWN;
    }
    lv_obj_invalidate(obj);
...
{
    lv_analogclock_t *analogclock = (lv_analogclock_t *)obj;
    if (analogclock->dial && analogclock->dial->src) {
        release_dial_src(analogclock->dial);
    }
    ...
    if (src_type == LV_IMG_SRC_VARIABLE) {
        ...
        dial->src = NULL;
        dial->src_type = LV_IMG_SRC_UNKNOWN;
    }
    lv_obj_invalidate(obj);
}
...
static void lv_analogclock_destructor(const lv_obj_class_t *class_p, lv_obj_t *obj)
{
    ...
    _LV_LL_READ(&analogclock->indicator_ll, indic) {
        if (indic->type == LV_analogclock_INDICATOR_TYPE_NEEDLE_IMG && indic->type_data.needle_img.src) {
            release_indicator_src(indic);
        }
    }
    lv_analogclock_dial_t *dial;
    _LV_LL_READ(&analogclock->dial_ll, dial) {
        if (dial->src) {
            release_dial_src(dial);
        }
    }
...
static void lv_analogclock_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
    ...
    if (code == LV_EVENT_DRAW_MAIN) {
        ...
        if (!analogclock->hide_point) {
            lv_coord_t r_edge_x = lv_area_get_width(&scale_area) / 2;
            lv_coord_t r_edge_y = lv_area_get_height(&scale_area) / 2;
            lv_point_t scale_center;
            scale_center.x = scale_area.x1 + r_edge_x;
            scale_center.y = scale_area.y1 + r_edge_y;
...
static void draw_arcs(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
    ...
    lv_coord_t r_out_x = lv_area_get_width(scale_area) / 2;
    lv_coord_t r_out_y = lv_area_get_height(scale_area) / 2;
    lv_coord_t r_out = LV_MIN(r_out_x, r_out_y);
    lv_point_t scale_center;
    scale_center.x = scale_area->x1 + r_out_x;
    scale_center.y = scale_area->y1 + r_out_y;
...
static void draw_ticks_and_labels(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, const lv_area_t *scale_area)
{
    ...
    lv_coord_t r_edge_x = lv_area_get_width(scale_area) / 2;
    lv_coord_t r_edge_y = lv_area_get_height(scale_area) / 2;
    lv_coord_t r_edge = LV_MIN(r_edge_x, r_edge_y);
    p_center.x = scale_area->x1 + r_edge_x;
    p_center.y = scale_area->y1 + r_edge_y;
...
static void release_indicator_src(lv_analogclock_indicator_t *indic)
{
    uint8_t src_type = indic->type_data.needle_img.src_type;
    if (src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_SYMBOL || src_type == LV_IMG_SRC_BIN) {
        lv_mem_free((void *)indic->type_data.needle_img.src);
        indic->type_data.needle_img.src = NULL;
        indic->type_data.needle_img.src_type = LV_IMG_SRC_UNKNOWN;
    }
}
static void release_dial_src(lv_analogclock_dial_t *dial)
{
    uint8_t src_type = dial->src_type;
    if (src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_SYMBOL || src_type == LV_IMG_SRC_BIN) {
        lv_mem_free((void *)dial->src);
        dial->src = NULL;
        dial->src_type = LV_IMG_SRC_UNKNOWN;
    }
}
                           ...
                           lv_coord_t pivot_x, lv_coord_t pivot_y)
{
    ...
    if (*indic_ptr && (*indic_ptr)->type == LV_analogclock_INDICATOR_TYPE_NEEDLE_IMG) {
        release_indicator_src(*indic_ptr);
    }

    *indic_ptr = lv_analogclock_add_needle_img(obj, analogclock->scale, src, pivot_x, pivot_y);
}
#endif
