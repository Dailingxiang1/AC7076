...
extern void bt_addr2string(u8 *addr, char *buf);
...
{
    /* printf("%s\n", __func__); */
#if TCFG_PSRAM_DEV_ENABLE && TCFG_PAY_TRANSITCODE_ENABLE && TCFG_TRANSIT_USE_PSRAM
...
#if TCFG_PSRAM_DEV_ENABLE && TCFG_TRANSIT_USE_PSRAM
size_t quickjs_malloc_usable_size(void *ptr)
{
    extern size_t malloc_psram_usable_size(void *pv);
    return malloc_psram_usable_size(ptr);
}

void *quickjs_malloc(size_t size)
{
    return malloc_psram(size);
}

void *quickjs_realloc(void *ptr, size_t size)
{
    extern void *realloc_psram(void *pv, size_t size);
    return realloc_psram(ptr, size);
}

void quickjs_free(void *pv)
{
