...
void upay2ali_send_event(void)
{
    ...
    u8 name = app_get_current_mode_name();
    app_send_message(APP_MSG_ALIPAY_U2A, name);
}
...
void ali2upay_send_event(void)
{
    extern u8 app_get_current_mode_name();
    u8 name = app_get_current_mode_name();
    app_send_message(APP_MSG_ALIPAY_A2U, name);
}
...
static int alipay_app_msg_handler(int *msg)
{
    switch (msg[0]) {
    case APP_MSG_ALIPAY_A2U:
        ali2upay_event_handler();
        break;
    case APP_MSG_ALIPAY_U2A:
...
APP_MSG_HANDLER(alipay_app_msg_entry) = {
    .owner      = 0xff,
    .from       = MSG_FROM_APP,
