...
void dev_manager_list_check_mount(void)
{
	...
	list_for_each_entry(dev, &__this->list, entry) {
        ...
        if(!strcmp(dev->parm->logo,"res_nor_mode")){
            continue;
        }
		if(!strcmp(dev->parm->logo,"nandflash_ftl")){
            continue;
        }
#if defined TCFG_NANDFLASH_UI_FAT_LOGO
        if(!strcmp(dev->parm->logo,TCFG_NANDFLASH_UI_FAT_LOGO)){
            continue;
        }
#endif
...
static void dev_manager_task(void *p)
{
	...
	dev_manager_add("res_nor_mode");
    dev_manager_set_valid_by_logo("res_nor_mode", 0);///将设备设置为无效设备
#endif
    ...
    /* ASSERT(!ret); */
#if TCFG_NANDFLASH_UNUSED_FILE_SYSTEM
	//不挂载文件系统，只使用ftl
	dev_open("nandflash_ftl",NULL);
#else
	...
	dev_manager_add(TCFG_NANDFLASH_UI_FAT_LOGO);
	dev_manager_set_valid_by_logo(TCFG_NANDFLASH_UI_FAT_LOGO, 0);///将设备设置为无效设备
#endif
