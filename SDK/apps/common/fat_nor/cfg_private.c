...
static RESFILE *cfg_private_create(const char *path, const char *mode)
{
    ...
    part_path = (char *)zalloc(path_len + 1);
    ASSERT(part_path);
    memcpy(part_path, path, path_len);
    ...
    tzflash_read((void *)buf_temp, align_size, attrs.sclust);
    for (; i < CFG_PRIVATE_FILE_NUM; i++) {
        int addr = attrs.sclust + i * sizeof(SDFILE_FILE_HEAD);
        tzflash_read((void *)&head, sizeof(SDFILE_FILE_HEAD), addr);
    ...
    tzflash_write((void *)buf_temp, align_size, attrs.sclust);
...
void cfg_private_erase(u32 addr, u32 len)
{
    ...
    while (erase_total_size) {
        //擦除区域操作
        tzflash_ioctl(erase_cmd, erase_addr);
...
int cfg_private_close(RESFILE *file)
{
    if (cfg_private_create_flag) {
        ...
        char *buf_temp = (char *)malloc(align_size);
        tzflash_read((void *)buf_temp, align_size, cfg_private_create_sclust);
        for (; i < CFG_PRIVATE_FILE_NUM; i++) {
        ...
        if (i < CFG_PRIVATE_FILE_NUM) {
            ...
            memcpy(buf_temp + i * sizeof(SDFILE_FILE_HEAD), &head, sizeof(SDFILE_FILE_HEAD));
            tzflash_write((void *)buf_temp, align_size, cfg_private_create_sclust);
        }
        if (buf_temp) {
            free(buf_temp);
        }
