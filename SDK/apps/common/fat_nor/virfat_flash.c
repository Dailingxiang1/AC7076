...
#if(TCFG_VIRFAT_FLASH_DEV_PATY_SIZE> FLASH_SIZE_8M)
#define CONFIG_FAT_FLASH_SIZE    (((TCFG_VIRFAT_FLASH_DEV_PATY_SIZE)+FLASH_SIZE_16M-1)/FLASH_SIZE_16M*FLASH_SIZE_16M)
#else
#define CONFIG_FAT_FLASH_SIZE    TCFG_VIRFAT_FLASH_DEV_PATY_SIZE
#endif
#else
...
static u16 fast_write_timer = 0;
static OS_MUTEX virfat_fast_write_mutex;
#define FAST_WRITE_TIMEOUT 100
#define FAST_WRITE_TIMER_ENABLE 0
#define virfat_fast_write_mutex_init() 	os_mutex_create(&virfat_fast_write_mutex)
#define virfat_fast_write_mutex_pend() 	os_mutex_pend(&virfat_fast_write_mutex, portMAX_DELAY)
#define virfat_fast_write_mutex_post() 	os_mutex_post(&virfat_fast_write_mutex)
#endif
...
static void virfat_fast_write_free(void *p)
{
#if VIRFAT_FLASH_FAST_WRITE
    virfat_fast_write_mutex_pend();
    virfat_flash_flush();
...
static u32 virfat_flash_write(void *buf, u32 addr_sec, u32 len)
{
    ...
    if (addr_sec == 0) {
...
#if VIRFAT_FLASH_FAST_WRITE
        virfat_fast_write_mutex_pend();
        if (FAST_WRITE_TIMER_ENABLE) {
            if (fast_write_timer) {
                sys_timeout_del(fast_write_timer);
                fast_write_timer = 0;
            }
        }
        if (!buf_write) {
        ...
        wlen = len;
        if (FAST_WRITE_TIMER_ENABLE) {
            fast_write_timer = sys_timeout_add(NULL, virfat_fast_write_free, FAST_WRITE_TIMEOUT);
        }
        virfat_fast_write_mutex_post();
#else
...
#if VIRFAT_FLASH_FAST_WRITE
int virfat_flash_fast_write_init(void)
{
    virfat_fast_write_mutex_init();
    return 0;
}
early_initcall(virfat_flash_fast_write_init);
#endif
...
#if VIRFAT_FLASH_FAST_WRITE
    virfat_fast_write_mutex_pend();
    if (virfat_addr && buf_write && fd) {
        virfat_ioctl(fd, IOCTL_ERASE_SECTOR, virfat_addr);
        int wlen = virfat_write(fd, buf_write, virfat_addr, 4096);
        if (wlen != 4096) {
            /* r_printf(">>>[test]:wlen = %d, len = %d\n", wlen, len); */
            log_e("write error!!!!!!!!!!!!!");
        }
...
#if VIRFAT_FLASH_FAST_WRITE
    virfat_fast_write_mutex_pend();
    if (FAST_WRITE_TIMER_ENABLE == 0) {
        virfat_fast_write_free(NULL);
    }
    virfat_fast_write_mutex_post();
#endif
    dev_close(device);
...
static int _private_sfc_write(void *fd, void *buf, u32 addr, u32 len)
{
    ...
    /* put_buf(buf, len); */
    wlen = tzflash_write(buf, len, addr);
    return wlen;
}
...
u32 virfat_flash_capacity()
{
    return 0;
}
u32 virfat_flash_get_real_capacity()
{
    return 0;
}
#endif
