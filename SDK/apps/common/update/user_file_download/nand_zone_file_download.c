...
#define NAND_FLASH_UI_ZONE_SIZE		(CONFIG_EXTERN_FLASH_SIZE)
...
#endif
    if (__this && __this->en) {
        rlen = ftl_write_bytes(addr, (u8 *)buf, len, &error);
    }
    return rlen;
}
        ...
        u32(**local_erase_hdl)(u32 cmd, u32 addr))
{
    ...
    if (local_read_hdl && local_write_hdl && local_erase_hdl) {
        ...
        *local_read_hdl = (u32(*)(void *buf, u32 addr, u32 len))nand_zone_file_read;
        *local_write_hdl = (u32(*)(void *buf, u32 addr, u32 len))nand_zone_file_write;
        *local_erase_hdl = (u32(*)(u32 cmd, u32 addr))nand_zone_file_erase;
    }
