...
#if (USER_FILE_UPDATE_V2_EN && TCFG_UPDATE_RESOURCE_EN)
...
#if RCSP_MODE
            extern void app_rcsp_prepare_update_ex_flash(void);
            app_rcsp_prepare_update_ex_flash();
#endif
        }
        // 入链表
...
static void user_file_update_write_task(void *p)
{
    ...
    while (0 == write_info->ret) {
        ...
        if (total_len) {
            u32 cur_progress = (total_len - (write_info->write_offset - write_info->write_program_addr)) * 100 / total_len;
            if (progress != cur_progress) {
                progress = cur_progress;
                UI_MSG_POST("upgrade:process=%4", cur_progress);
            }
        } else {
            total_len = write_info->write_offset - write_info->write_program_addr + write_info->write_len;
        }
...
extern u16 CRC16_with_initval(const void *ptr, u32 len, u16 i_val);

u16 calc_crc16_with_init_val(u16 init_crc, u8 *ptr, u16 len)
{
    return CRC16_with_initval(ptr, len, init_crc);
}
...
static int user_file_download_all_res_loop(program_info_t *info, user_chip_update_info_t *update_info_entry, u32(*verify_data_read_func)(void *buf, u32 addr, u32 len))
{
    ...
    do {
        ...
        //记录升级文件数量，现阶段只用mode.bin watch.bin两个文件
        ++file_num;
        if (file_num > 2) {
            log_error("<%s> file_num %d is error", __func__, file_num);
        }
        UI_MSG_POST("upgrade:file_num=%4", file_num);
        if (file_num >= 2) {
            file_num = 0;
        }
...
#else // (USER_FILE_UPDATE_V2_EN && TCFG_UPDATE_RESOURCE_EN)
...
#endif // (USER_FILE_UPDATE_V2_EN && TCFG_UPDATE_RESOURCE_EN)
