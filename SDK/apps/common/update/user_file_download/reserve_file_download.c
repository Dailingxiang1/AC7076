...
static int record_reserved_file_info(u8 *file_name, u32 start_addr, u32 len, u32 res_end)
{
    ...
    record_reserved_file_info->res_end = res_end;
...
static int norflash_f_open(u32 file_len, u8 *file_name)
{
    ...
    u8 file_path_len = sizeof(SDFILE_EXT_RESERVED_ROOT_PATH) + strlen((const char *)file_name);
    ...
    if (!fp) {
        ...
        memcpy((void *)(file_path + strlen(SDFILE_EXT_RESERVED_ROOT_PATH)), file_name, strlen((const char *)file_name));
        fp = fopen((const char *)file_path, "r");
    }
...
_ERR_RET:
    // 先记录所有需要升级的预留区域
    record_reserved_file_info(file_name, __this->info.dev_addr, __this->info.dev_addr ? file_attr.fsize : 0, __this->info.dev_addr + file_len);
        ...
        u32(**local_erase_hdl)(u32 cmd, u32 addr))
{
    ...
    if (local_read_hdl && local_write_hdl && local_erase_hdl) {
        ...
        *local_read_hdl = (void *)reserve_file_sfc_read;
        *local_write_hdl = (void *)reserve_file_sfc_write;
        *local_erase_hdl = (void *)reserve_file_sfc_erase;
    }
...
static int reserve_update_release(user_chip_update_info_t *update_info_entry)
{
    if (update_info_entry) {
        ...
        // free(update_info_entry); //bug! todo...
    }
...
void get_remote_reserved_info_during_update_detail(u8 *reserved_name, u32 reserved_addr, u32 reserved_len, u8 opt)
{
    ...
    if (0 == memcmp(reserved_name, g_record_reserved_file_info->reserved_name, strlen((const char *)reserved_name) + 1)) {
        record_reserved_file_info_verify(g_record_reserved_file_info, reserved_addr, reserved_len, opt);
    } else if (!list_empty(&g_record_reserved_file_info->entry)) {
        list_for_each_entry(item, &g_record_reserved_file_info->entry, entry) {
            if (0 == memcmp(item->reserved_name, reserved_name, strlen((const char *)reserved_name) + 1)) {
...
int get_remote_reserved_is_update(u8 *reserved_name, u32 reserved_addr, u32 reserved_len)
{
    record_reserved_file_info_t *item = NULL;
    if (NULL == g_record_reserved_file_info) {
        return 0;
    ...
    // 把已记录了需要升级的预留区域信息，与远端flash.bin中的预留区域对比，筛选出远端真正需要升级的预留区域
    if (0 == memcmp(reserved_name, g_record_reserved_file_info->reserved_name, strlen((const char *)reserved_name) + 1)) {
        item = g_record_reserved_file_info;
        if (item->len == reserved_len && item->start_addr == reserved_addr) {
            return 1;
        }
    }
    list_for_each_entry(item, &g_record_reserved_file_info->entry, entry) {
...
int reserved_area_info_upgrade(u32 *base_addr, u32 *total_size)
{
    record_reserved_file_info_t *item = NULL;
    if (NULL == g_record_reserved_file_info) {
        return 0;
    }
    u32 res_addr = *base_addr;
    u32 res_size = *total_size;
    item = g_record_reserved_file_info;
    if ((res_addr >= item->start_addr) && (item->res_end < (res_addr + res_size))) {
        *base_addr = item->res_end;
        *total_size = res_addr + res_size - item->res_end;
        return 0;
    }
    list_for_each_entry(item, &g_record_reserved_file_info->entry, entry) {
        if ((res_addr >= item->start_addr) && (item->res_end < (res_addr + res_size))) {
...
// 0 - 检测远端有没有预留区域数据需要升级，1 - 强制loader不升级预留区域
bool reserved_area_special_handle(u8 opt)
{
    if (opt) {
        reserved_update_in_ota_flag = 1;
    } else if (g_record_reserved_file_info) {
        return true;
    }
    return false;
}
