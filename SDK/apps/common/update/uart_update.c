...
void uart_data_decode(u8 *buf, u32 len);
...
void uart_update_init(uart_update_cfg *cfg)
{
    ...
    uart_hw_init(update_cfg, (void *)uart_data_decode);
    printf(">>>%s successful\n", __func__);
}
...
LSB_CRITICAL_HANDLE_REG(uart_update, clock_critical_enter, clock_critical_exit)
