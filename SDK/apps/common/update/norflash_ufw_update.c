...
#pragma bss_seg(".norflash_ufw_update.data.bss")
#pragma data_seg(".norflash_ufw_update.data")
#pragma const_seg(".norflash_ufw_update.text.const")
#pragma code_seg(".norflash_ufw_update.text")
#endif
...
static u16 norflash_ufw_update_f_read(void *fp, u8 *buff, u16 len)
{
    ...
    if (dev_ptr) {
        wdt_clear();
        dev_bulk_read(dev_ptr, buff, user_remote_device_get_cur_addr(), len);
...
*/
/*----------------------------------------------------------------------------*/
static s32 norflash_update_file_read(void *buf, u32 addr, u32 len)
{
    int rlen = 0;
...
#endif // TCFG_COMPELTE_UFW_NOR_EN
