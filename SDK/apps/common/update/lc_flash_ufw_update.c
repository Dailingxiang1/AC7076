...
/* #define SDFILE_EXT_RES_ROOT_PATH       	SDFILE_MOUNT_PATH"/EXT_RESERVED/"  //拓展预留区域分区路径 */
#define SDFILE_EXT_RES_ROOT_PATH       	SDFILE_MOUNT_PATH"/app/"  //普通预留区域分区路径
...
static u16 lc_flash_ufw_update_f_open(void)
{
    ...
    if (dev_ptr) {
        ...
        printf("open %s file succ 0x%x, 0x%x", path, (u32)dev_ptr, user_remote_base_addr);
        return 1;
    } else {
...
static u16 lc_flash_ufw_update_f_read(void *fp, u8 *buff, u16 len)
{
    ...
    if (dev_ptr) {
        wdt_clear();
        rlen = fread(buff, len, 1, (FILE *)dev_ptr);
    }
    return (u16)rlen;
}
...
*/
/*----------------------------------------------------------------------------*/
static s32 lc_flash_update_file_read(void *buf, u32 addr, u32 len)
{
    int rlen = 0;
    ...
    if (dev_ptr) {
        ...
        rlen = fread(buf, len, 1, (FILE *)dev_ptr);
    }
    return (u32)rlen;
}
...
{
    if (support_norflash_update_loader_only == 0) {
        goto _ERR_RET;
    }
    printf("%s, %d, %s\n", __func__, __LINE__, reserved_area_name);
    if (NULL == path) {
        path = zalloc(sizeof(SDFILE_EXT_RES_ROOT_PATH) + strlen(reserved_area_name));
        if (NULL == path) {
            printf("err: alloc lc flash ufw path fail\n");
    ...
    exflash_move_loader_task_init(&lc_flash_ufw_update_info);
