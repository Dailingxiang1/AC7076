...
int music_player_end_deal(struct music_player *player_hd, int parm)
{
    ...
    if (read_err) {
        ...
        if (player_hd->fsn == NULL) {
            log_e("player_hd->fsn is NULL\n");
            return MUSIC_PLAYER_ERR_INVALID_STATE;
        }
...
int music_player_get_play_status(void)
{
    struct file_player *file_player = get_music_file_player();
    if (!file_player) {
        return FILE_PLAYER_STOP;
    }
    return music_file_get_player_status(file_player);
}
...
char *music_player_get_cur_music_dev(struct music_player *player_hd)
{
    if (player_hd) {
        ...
        if (logo) {
            char *str = strstr(logo, "_rec");
            if (str) {
                int log_len = strlen(logo) - strlen(str);
                char *music_dev_logo = malloc(log_len + 1);
                ASSERT(music_dev_logo);
                //录音设备,切换到音乐设备播放
                strncpy(music_dev_logo, logo, log_len);
                music_dev_logo[log_len] = '\0';
...
int music_player_play_record_folder(struct music_player *player_hd, char *logo, struct __breakpoint *bp)
{
    ...
    if (bp) {
        ...
        //start decoder
        err = music_player_decode_start(player_hd, player_hd->file, 0);//录音文件夹不支持断点播放
    }
    if (err == MUSIC_PLAYER_SUCC) {
        //选定新设备播放成功后，需要激活当前设备
        dev_manager_set_active(player_hd->dev);
