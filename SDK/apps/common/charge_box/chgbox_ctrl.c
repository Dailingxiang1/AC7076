...
#pragma bss_seg(".chgbox_ctrl.data.bss")
#pragma data_seg(".chgbox_ctrl.data")
#pragma const_seg(".chgbox_ctrl.text.const")
#pragma code_seg(".chgbox_ctrl.text")
#endif
...
#define CHGBOX_FORCE_CHARGE_TIMES 120 //耳机电池不带保护建议设置2min
...
extern void usb_wakeup_callback(P33_IO_WKUP_EDGE edge);
extern OS_MUTEX power_mutex;
#if (TCFG_HALL_PORT != NO_CONFIG_PORT)
static const struct _p33_io_wakeup_config hall_det_port = {
    .edge               = RISING_EDGE,        //唤醒方式选择,可选：上升沿\下降沿\双边沿
    .filter      		= PORT_FLT_4ms,
    .gpio               = TCFG_HALL_PORT,     //唤醒口选择
    .callback			= hall_wakeup_deal,
};
#endif  /* (TCFG_HALL_PORT != NO_CONFIG_PORT) */
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_WIRELESS_ENABLE)
struct _p33_io_wakeup_config wl_det_port = {
    .edge       = RISING_EDGE,                             //唤醒方式选择,可选：上升沿\下降沿
    .filter             = PORT_FLT_4ms,                                    //滤波使能
    .gpio      = TCFG_WL_AD_DET_IO,                       //唤醒口选择
    .callback			= NULL,
};
#endif

#if (TCFG_CHARGE_BOX_ENABLE && TCFG_SHORT_PROTECT_ENABLE)
struct _p33_io_wakeup_config short_det_port = {
    .edge       = FALLING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .filter             = PORT_FLT_4ms,                                    //滤波使能
    .gpio      = TCFG_SHORT_PROTECT_IO,                   //唤醒口选择
    .callback			= NULL,
};
#endif

#if (TCFG_CHARGE_FULL_DET_IO != NO_CONFIG_PORT)
struct _p33_io_wakeup_config charge_full_det_port = {
    .edge       = RISING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .filter             = PORT_FLT_4ms,                                    //滤波使能
    .gpio      = TCFG_CHARGE_FULL_DET_IO,                   //唤醒口选择
    .callback			= NULL,
};
#endif  /* (TCFG_CHARGE_FULL_DET_IO != NO_CONFIG_PORT) */
struct _p33_io_wakeup_config usb_det_port = {
    .edge       = BOTH_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .filter             = PORT_FLT_16ms,                                    //滤波使能
    .gpio      = TCFG_SCREEN_USB_PIN,                   //唤醒口选择
    .callback			= usb_wakeup_callback,
};
int hall_port_wakeup_init(void)
{
#if (TCFG_HALL_PORT != NO_CONFIG_PORT)
    p33_io_wakeup_port_init(&hall_det_port);
    p33_io_wakeup_enable(TCFG_HALL_PORT, 1);
#endif
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_WIRELESS_ENABLE)
    p33_io_wakeup_port_init(&wl_det_port);
    p33_io_wakeup_enable(TCFG_WL_AD_DET_IO, 1);
#endif
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_SHORT_PROTECT_ENABLE)
    p33_io_wakeup_port_init(&short_det_port);
    p33_io_wakeup_enable(TCFG_SHORT_PROTECT_IO, 1);
#endif
#if (TCFG_CHARGE_FULL_DET_IO != NO_CONFIG_PORT)
    p33_io_wakeup_port_init(&charge_full_det_port);
    p33_io_wakeup_enable(TCFG_CHARGE_FULL_DET_IO, 1);
#endif
    p33_io_wakeup_port_init(&usb_det_port);
    p33_io_wakeup_enable(TCFG_USB_ONLE_DET_IO, 1);
    return 0;
}

platform_initcall(hall_port_wakeup_init);
...
void chgbox_enter_soft_power_off(void)
{
...
#if (TCFG_CHARGE_BOX_ENABLE && TCFG_SHORT_PROTECT_ENABLE)
    // power_wakeup_index_enable(7, 0);
    p33_io_wakeup_enable(TCFG_SHORT_PROTECT_IO, 0);
#endif
    ...
    if ((sys_info.init_ok == 0) || (sys_info.current_limit == 1)) {
        ...
        p33_io_wakeup_enable(TCFG_HALL_PORT, 0);
...
u8 chargebox_check_output_short(void)
{
    ...
    gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_L_PORT), PORT_INPUT_PULLUP_10K);
    gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_R_PORT), PORT_INPUT_PULLUP_10K);
...
static int chargebox_common_event_handler(struct chargebox_event *e)
{
    ...
    case CHGBOX_EVENT_EXIT_TEMP_PROTECT:
        ...
        if (sys_info.charge) {
            ...
            if (sys_info.current_limit) {
                ...
                if (ear_info.online[EAR_L]) {
                    sbox_left_charging_set(1);
                }
                if (ear_info.online[EAR_R]) {
                    sbox_right_charging_set(1);
                }
            } else if (msg_flag == 0) {
        ...
        break;
#endif
    case CHGBOX_EVENT_EXCHANGE_MAC: {
        printf("    KEY_EXCHANGE_MAC \n");
#if TCFG_CHARGE_BOX_ENABLE
        extern u8 app_chargebox_api_exchange_addr(void);
        app_chargebox_api_exchange_addr();
    ...
    case CHGBOX_EVENT_USB_OUT:
        ...
        if (app_get_curr_task() != APP_IDLE_TASK) {
            app_task_switch_to(APP_IDLE_TASK, 0);
        }
#endif
        break;
    case CHGBOX_EVENT_OPEN_LID:
        gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_L_PORT), PORT_OUTPUT_LOW);
        gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_R_PORT), PORT_OUTPUT_LOW);
        os_time_dly(10);
        gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_L_PORT), PORT_OUTPUT_HIGH);
        gpio_set_mode(IO_PORT_SPILT(TCFG_CHARGEBOX_R_PORT), PORT_OUTPUT_HIGH);
        break;
    case CHGBOX_EVENT_HANDSHAKE_OK:
        ...
        if (app_get_curr_task() != APP_PC_TASK) {
            app_task_switch_to(APP_PC_TASK, 0);
        }
#endif
        break;
    }
...
static void charge_app_lid_close_deal(void)
{
    ...
    log_debug("func<%s>  rets:%x LID_CNT:%d\n", __func__, rets, sys_info.lid_cnt);
    if (sys_info.lid_cnt & BIT(7)) {
        if (sys_info.lid_cnt & (~BIT(7))) {
            ...
            u8 msg_flag = chargebox_check_output_short();
            log_info("msg_flag:%d\n", msg_flag);
            log_info("current_limit:%d\n", sys_info.current_limit);
...
static void charge_app_send_power(void)
{
    if (sys_info.shut_cnt || sys_info.lid_cnt || sys_info.earfull || sys_info.force_charge) {
        log_debug("%s shut_cnt:0x%x lid_cnt:0x%x earfull:0x%x force_charge:%x",
                  __func__, sys_info.shut_cnt, sys_info.lid_cnt, sys_info.earfull, sys_info.force_charge);
        return;
    }
...
{
    /*TODO 调试阶段 临时屏蔽*/
    printf("%s error!!! TODO debug", __func__);
    return;
...
#endif
        extern void app_chargebox_timer_del(void);
        app_chargebox_timer_del();
        if (sys_info.earfull) {
#if TCFG_CHARGGBOX_AUTO_SHUT_DOWN_TIME
...
static int charge_chargebox_event_handler(struct chargebox_event *e)
{
    ...
    case CHGBOX_EVENT_OPEN_LID:
        ...
        sbox_state_handler(CHGBOX_EVENT_OPEN_LID);
#endif
#if(RCSP_MODE && (RCSP_MODE !=  RCSP_MODE_WATCH))
        //bt_ble_rcsp_adv_enable();
#endif
        sbox_left_charging_set(0);
        sbox_right_charging_set(0);
        chargebox_set_newstatus(CHG_STATUS_COMM);     //设置新模式
        break;
    ...
    case CHGBOX_EVENT_EAR_L_ONLINE:
        log_info("EAR_L_IN_1\n");
#if TCFG_EARPHONE_PROTOCOL
        sbox_state_handler(CHGBOX_EVENT_EAR_L_ONLINE);
#endif
...
#if TCFG_EARPHONE_PROTOCOL
        sbox_state_handler(CHGBOX_EVENT_EAR_L_OFFLINE);
#endif
    ...
    case CHGBOX_EVENT_EAR_R_ONLINE:
        log_info("EAR_R_IN_1\n");
#if TCFG_EARPHONE_PROTOCOL
        sbox_state_handler(CHGBOX_EVENT_EAR_R_ONLINE);
#endif
...
#if TCFG_EARPHONE_PROTOCOL
        sbox_state_handler(CHGBOX_EVENT_EAR_R_OFFLINE);
#endif
...
void ex_clid_trig(void)
{
    ...
    /*  */
    /*     int msg[3]; */
    /*     msg[0] = (int)exit_clid_trig_func; */
    /*     msg[1] = 0; */
    /*     os_taskq_post_type("ui", Q_CALLBACK, 2, msg); */
    /* } */
}

extern u8 first_check_online;
...
#if TCFG_EARPHONE_PROTOCOL
        sbox_state_handler(CHGBOX_EVENT_CLOSE_LID);
#endif
#if RCSP_MODE
        //bt_ble_rcsp_adv_disable();
#endif
...
{
    log_debug("%s cnt:0x%x", __func__, sys_info.shut_cnt);
    if (sys_info.shut_cnt & BIT(7)) {
        if (sys_info.shut_cnt & (~BIT(7))) {
            sys_info.shut_cnt--;
            app_chargebox_send_mag(CHGBOX_MSG_SEND_SHUTDOWN);
...
{
    log_debug("%s cnt:0x%x", __func__, sys_info.lid_cnt);
    if (sys_info.lid_cnt & BIT(7)) {
        if (sys_info.lid_cnt & (~BIT(7))) {
            sys_info.lid_cnt--;
            app_chargebox_send_mag(CHGBOX_MSG_SEND_CLOSE_LID);
...
int charge_box_key_event_handler(u16 event)
{
    ...
    if (sys_info.chgbox_status == CHG_STATUS_CHARGE) {
        ...
        case KEY_BOX_POWER_FIVE:
            ...
            if (app_get_curr_task() == APP_IDLE_TASK) {
                bt_auto_off_cnt = CHGBOX_BT_AUTO_OFF_TIMES;
                app_task_switch_to(APP_BT_TASK, 0);
            }
#endif
            break;
        default:
...
static void charge_app_poweroff_uninit(void)
{
    log_info("%s", __func__);
...
platform_uninitcall(charge_app_poweroff_uninit);


#elif (TCFG_CHARGE_IC_MANAGER_MODE_ENABLE)

int charge_box_ctrl_event_handler(struct chargebox_event *chg_event)
{
    log_info("func:%s , is empty\n");
    return 0;
}
void chgbox_init_app(void)
{
