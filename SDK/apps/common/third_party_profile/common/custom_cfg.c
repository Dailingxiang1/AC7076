...
#define USE_FS_API_SEL						SDFILE_API_AC693X					//SDFILE_API_MANIFEST
...
extern int res_sdfile_update_exif_region(u32 *base, u32 *len);
extern int reusable_exif_region(u32 *base, u32 *len);
...
static void ex_cfg_get_addr_and_len(u32 *addr, u32 *len)
{
    ...
    if (fp) {
        ...
        *len = 0;
        ...
        cfg_printf("try reusable region\n");
        if (reusable_exif_region(addr, len) != 0) {
            *addr = 0;
            *len = 0;
        }
#endif
#if CONFIG_RESFS_UPDATE_ENABLE
        cfg_printf("try reusable region\n");
        if (res_sdfile_update_exif_region(addr, len) != 0) {
            *addr = 0;
            *len = 0;
        }
#endif
...
u32 ex_cfg_get_start_addr(void)
{
    ...
    if (fp) {
        ...
        u32 len;
        cfg_printf("try reusable region\n");
        if (reusable_exif_region(&start_addr, &len) != 0) {
            start_addr = 0;
        }
#endif
#if CONFIG_RESFS_UPDATE_ENABLE
        u32 len;
        cfg_printf("try reusable region\n");
        if (res_sdfile_update_exif_region(&start_addr, &len) != 0) {
            start_addr = 0;
        }
#endif
    }
