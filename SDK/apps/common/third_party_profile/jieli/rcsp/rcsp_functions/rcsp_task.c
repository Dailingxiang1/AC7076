...
static void app_rcsp_action_end_callback(void)
{
    ...
    if (app_get_curr_task() == APP_RCSP_ACTION_TASK) {
        ...
        app_mode_stack_clr(APP_RCSP_ACTION_TASK);
...
{
    if (BT_MODULES_IS_SUPPORT(BT_MODULE_CLASSIC)) {
...
static int app_rcsp_task_event_handle(int *msg)
{
    switch (action_prepare.action)		{
    case RCSP_TASK_ACTION_FILE_TRANSFER:
        break;
    ...
    case RCSP_TASK_ACTION_WATCH_TRANSFER:
#if JL_RCSP_EXTRA_FLASH_OPT
        return rcsp_extra_flash_event_deal(msg);
#endif
        break;
    case RCSP_TASK_ACTION_BLUK_TRANSFER:
        break;
...
void app_rcsp_task_prepare(u8 type, u8 action, u8 OpCode_SN)
{
    ...
    /* printf("%s mode_task:%d type:%d action:%d \n", __func__, app_get_curr_task(), type, action); */
    task_switch_flag ++;
...
struct app_mode *app_enter_rcsp_mode(int arg)
{
    int msg[16];
    ...
    while (1) {
        ...
        if (app_rcsp_task_event_handle(msg) == false) {
