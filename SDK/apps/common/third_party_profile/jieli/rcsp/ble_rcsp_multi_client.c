...
#define LOG_TAG_CONST       RCSP_CLIENT
#define LOG_TAG     		"[ble mul cli]"
...
#define RCSP_ANCS_AMS_EN  			  	 1
#if RCSP_ANCS_AMS_EN
#define SUPPORT_MAX_CLIENT           (RCSP_MULTI_BLE_MASTER_NUMS - 1)
...
static void client_create_connection_cannel(void *p);
...
static void ble_report_data_deal(att_data_report_t *report_data, target_uuid_t *search_uuid)
{
    ...
    case GATT_EVENT_NOTIFICATION:  //notify
        ...
        ret = ble_recv_musicdata_handler(report_data->blob, report_data->blob_length);
        //歌词不加密。如果是歌词，直接返回，不解密。
        if (ret) {
            break;
        }
        ret = ble_notify_recv_data_handler(report_data->blob, report_data->blob_length);
        if (ret) {
...
static void conn_pair_vm_do(struct pair_info_t *info, u8 rw_flag)
{
    ...
    if (rw_flag == 0) {
        ...
        for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
            ...
            log_info_hexdump(conn_pair_info_table[i].peer_address_info, 7);
        }
        ...
        if (!info) {
            ...
            log_info_hexdump((u8 *)conn_pair_info_table, unit_len * SUPPORT_MAX_CLIENT);
            log_info_hexdump((u8 *)info, unit_len);
            ...
            log_info_hexdump(info->peer_address_info, 7);
...
{
    log_info("%s handle[%d]:%x %x", __func__, cid, client_con_handle[cid], state);
    if (!client_con_handle[cid] && state == BLE_ST_SEARCH_COMPLETE) {
        return;
    }
...
//------------------------------------------------------------
ble_state_e get_ble_work_state(u8 cid)
{
...
static void client_search_profile_start(u16 conn_handle)
{
    ...
    if (client_config->search_uuid_cnt) {
        ...
        log_info("search_profile_all :%d", i);
    } else {
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    //    log_info("rssi:%d\n",report_pt->rssi);
    /* log_error("report_pt->address_type: %d\n",report_pt->address_type); */
    /* log_info_hexdump(report_pt->address, 6); */
    for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
        if (conn_pair_info_table[i].pair_flag) {
            ...
            /* log_info_hexdump(report_pt->address, 6); */
            /* log_info_hexdump(&conn_pair_info_table[i].peer_address_info[1], 6); */
...
just_creat:
    if (find_tag) {
        ...
        if (timer_add_delete_connection) {
            ...
            timer_add_delete_connection = sys_timeout_add(NULL, client_create_connection_cannel, 5 * 1000);
        } else {
            timer_add_delete_connection = sys_timeout_add(NULL, client_create_connection_cannel, 5 * 1000);
        }
...
s8 mul_get_idle_dev_index()
{
    ...
    count = SUPPORT_MAX_CLIENT;
    log_info(">>%s<<\n", __func__);
    log_info_hexdump((u8 *)group_handle, count * 2);
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
...
#if TCFG_COLOR_SCREEN_CHARGING_CASE_ENABLE
                sbox_ble_connect_flag_set(BLE_CONNECTED);
                if (ble_timeout_page_timer != 0) {
                    sys_timeout_del(ble_timeout_page_timer);
                    ble_timeout_page_timer = 0;
                }
#endif
                if (packet[3]) {
                    log_info("LE_MASTER CREATE CONNECTION FAIL!!! %0x\n", packet[3]);
                ...
                log_info("cur_dev_cid:%d", cur_dev_cid);
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
...
#if TCFG_COLOR_SCREEN_CHARGING_CASE_ENABLE
            sbox_ble_connect_flag_set(BLE_DISCONNET);
            ...
            if (packet[5] == 0x3e) {
                ble_client_module_enable(1);//遇到0x3e类报错直接重新adv和scan
            }
#if (defined TCFG_COLOR_SCREEN_CHARGING_CASE_ENABLE) && TCFG_COLOR_SCREEN_CHARGING_CASE_ENABLE
            if (packet[5] == 0x08) {
                ble_client_module_enable(1);//遇到0x08类报错增加2分钟回链
                if (ble_timeout_page_timer == 0) {
                    ble_timeout_page_timer = sys_timeout_add(0, (void (*)(void *))ble_client_module_enable, TCFG_BT_TIMEOUT_PAGE_TIME * 1000);
                }
            }
#endif
        }
...
int bt_ble_scan_enable(void *priv, u32 en)
{
    u32 rets;
    __asm__ volatile("%0 = rets":"=r"(rets));
    log_info("func:%s() rets: 0x%x scan_ctrl_en:%d\n", __func__, rets, scan_ctrl_en);
    ...
    log_info("cur_state:%x next_state:%x", cur_state, next_state);
    switch (cur_state) {
    case BLE_ST_SCAN:
    case BLE_ST_IDLE:
    case BLE_ST_INIT_OK:
...
static int client_force_search(u8 onoff, s8 rssi)
{
    ...
    if (force_seach_onoff != onoff) {
        ...
        //强制搜索前后，关创建监听
        if (get_ble_work_state(cur_dev_cid) == BLE_ST_CREATE_CONN) {
            client_create_connection_cannel(NULL);
        }
    }
    return 0;
}
...
void rcsp_bt_multi_client_init()
{
    ...
    le_multi_client_hdl_init(cur_dev_cid);
#if TCFG_EARPHONE_PROTOCOL
    sbox_get_earphone_mac(custom_remote_addr);
    sbox_power_on_check();
#else
    ble_client_module_enable(1);
#endif
...
void ble_client_set_remote_addr(u8 *addr)
{
    ...
    log_info("%s:", __func__);
    memcpy(custom_remote_addr, addr, 6);
    log_info_hexdump(custom_remote_addr, 6);
}
#endif
...
int custom_client_write_without_respond_send_callback(u8 *data, u16 len)
{
    target_handle[cur_dev_cid].write_no_respond = 0x0082;
    return client_operation_send(target_handle[cur_dev_cid].write_no_respond, data, len, ATT_OP_WRITE_WITHOUT_RESPOND);
}
#endif //TCFG_EARPHONE_PROTOCOL
