...
struct _DEV_info {
    ...
    u32 flash_handle;
    u32 flash_handle_2;
    u32 nandflash_handle;
};
#pragma pack()
#pragma pack(1)
struct _DEV_info_sub {
    u8 status;
    u8 index;
    u32 handle;
};
struct dev_info_reuse {
    u8 type;
    u8 desc;
    u8 value;
};
#pragma pack()
#define DEV_INFO_DATA_OFF_STATUS			0
#define DEV_INFO_DATA_OFF_VERSION			1
#define DEV_INFO_DATA_OFF_LEN				2
#define DEV_INFO_DATA_OFF_STORAGE_TYPE		3
#define DEV_INFO_DATA_OFF_PAYLOAD			4
#define DEV_INFO_DATA_LEN_ONLINE		6
#define DEV_INFO_DATA_LEN_OFFLINE		2
...
static void common_function_attr_bt_emitter_connect_state_set(void *priv, u8 attr, u8 *data, u16 len, u16 ble_con_handle, u8 *spp_remote_addr)
{
    ...
    log_info("USER_MSG_RCSP_SET_BT_EMITTER_CONNECT_STATES, state = %d\n", data[0]);
    if (data[0]) {
#if TCFG_USER_EMITTER_ENABLE
        bt_search_stop();
#endif
        log_info_hexdump(rcspModel->emitter_con_addr, 6);
...
static u32 common_function_attr_dev_info_get(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    struct RcspModel *rcspModel = (struct RcspModel *) priv;
#if RCSP_MODE != RCSP_MODE_EARPHONE
#if !TCFG_NANDFLASH_DEV_ENABLE//norflash 使用旧协议
    ...
    if (rcspModel->file_transfer_mode && dev_manager_get_root_path_by_logo(TCFG_NANDFLASH_UI_FAT_LOGO)) {
        log_info("dev [ui_fat] online\n");
        dev_info.status |= BIT(RCSPDevMapFLASH);
        dev_info.flash_handle = app_htonl((u32)RCSPDevMapFLASH);
    }
#endif
#if (defined TCFG_NANDFLASH_FAT_ENABLE && TCFG_NANDFLASH_FAT_ENABLE)
    if (rcspModel->file_transfer_mode && dev_manager_get_root_path_by_logo(TCFG_NANDFLASH_FAT_ROOT)) {
        printf("dev [fat_nand] online\n");
        dev_info.status |= BIT(RCSPDevMapNANDFLASH);
        dev_info.nandflash_handle = app_htonl((u32)RCSPDevMapNANDFLASH);
    }
#endif
...
#else//nandflash 使用新协议

    u8 data_buf[4 + 7 * 6 + 3]; //head(4)+max_device_num(7)*device_info(6)+end(3);
    data_buf[DEV_INFO_DATA_OFF_STATUS] = 0xff;		//标识
    data_buf[DEV_INFO_DATA_OFF_VERSION] = 0x0;		//当前版本
    data_buf[DEV_INFO_DATA_OFF_STORAGE_TYPE] = 1;	//type1 - 设备信息
    int data_off = DEV_INFO_DATA_OFF_PAYLOAD;
    struct _DEV_info_sub dev_info = {0};
    if (dev_manager_get_root_path_by_logo("udisk0")) {
        printf("dev [udisk0] online\n");
        dev_info.status = 1;
        dev_info.index = RCSPDevMapUDISK;
        dev_info.handle = app_htonl((u32)RCSPDevMapUDISK);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
        data_off += DEV_INFO_DATA_LEN_ONLINE;
    } else {
        //不在线设备信息可以缺省
        /* dev_info.status = 0; */
        /* dev_info.index = RCSPDevMapUDISK; */
        /* memcpy(data_buf + data_off, & dev_info,DEV_INFO_DATA_LEN_OFFLINE); */
        /* data_off += DEV_INFO_DATA_LEN_OFFLINE; */
    }
    if (dev_manager_get_root_path_by_logo("sd0")) {
        printf("dev [sd0] online\n");
        dev_info.status = 1;
        dev_info.index = RCSPDevMapSD0;
        dev_info.handle = app_htonl((u32)RCSPDevMapSD0);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
        data_off += DEV_INFO_DATA_LEN_ONLINE;
    }
    ...
    if (dev_manager_get_root_path_by_logo("sd1")) {
        ...
        dev_info.status = 1;
        dev_info.index = RCSPDevMapSD1;
        dev_info.handle = app_htonl((u32)RCSPDevMapSD1);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
        data_off += DEV_INFO_DATA_LEN_ONLINE;
    }
    ...
    if (rcspModel->file_transfer_mode && dev_manager_get_root_path_by_logo("virfat_flash")) {
        ...
        dev_info.status = 1;
        dev_info.index = RCSPDevMapFLASH;
        dev_info.handle = app_htonl((u32)RCSPDevMapFLASH);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
        data_off += DEV_INFO_DATA_LEN_ONLINE;
    }
#if (defined  TCFG_NANDFLASH_UI_FAT_ENABLE&&TCFG_NANDFLASH_UI_FAT_ENABLE )
    if (rcspModel->file_transfer_mode && dev_manager_get_root_path_by_logo(TCFG_NANDFLASH_UI_FAT_LOGO)) {
        printf("dev [ui_fat] online\n");
        dev_info.status = 1;
        dev_info.index = RCSPDevMapFLASH;
        dev_info.handle = app_htonl((u32)RCSPDevMapFLASH);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
    ...
    if (rcspModel->file_transfer_mode && dev_manager_get_root_path_by_logo(TCFG_NANDFLASH_FAT_ROOT)) {
        printf("dev [fat_nand] online\n");
        dev_info.status = 1;
        dev_info.index = RCSPDevMapNANDFLASH;
        dev_info.handle = app_htonl((u32)RCSPDevMapNANDFLASH);
        memcpy(data_buf + data_off, & dev_info, DEV_INFO_DATA_LEN_ONLINE);
        data_off += DEV_INFO_DATA_LEN_ONLINE;
    }
#endif
    struct dev_info_reuse reuse = {
        .type = 2, //type 2 -- 复用信息
        .desc = 2,
        .value = 0,
    };
    memcpy(data_buf + data_off, &reuse, sizeof(struct dev_info_reuse));
    data_off += sizeof(struct dev_info_reuse);
    data_buf[DEV_INFO_DATA_OFF_LEN] = data_off - DEV_INFO_DATA_OFF_STORAGE_TYPE - sizeof(struct dev_info_reuse);
    /* printf("%s %d ",__func__,data_off); */
    /* put_buf(data_buf,data_off); */
    rlen = add_one_attr(buf, buf_size, offset, attr, (u8 *)data_buf, data_off);
#endif
#endif
    return rlen;
}
...
{
    log_info("rcsp_common_function_get, mask = %x\n", mask);
