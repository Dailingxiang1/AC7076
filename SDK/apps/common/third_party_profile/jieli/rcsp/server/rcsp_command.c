...
void find_decice_tws_connect_handle(u8 flag, u8 *param)
{
    ...
    u8 other_opt[3] = {0};
    /* log_info("rcsp_find %s, %s, %d, flag:%d\n", __FILE__, __FUNCTION__, __LINE__, flag); */
    /* log_info_hexdump(param, 2); */
    /* log_info_hexdump(param, 4); */
    /* log_info("rcsp_find %s, %s, %d, flag:%d\n", __FILE__, __FUNCTION__, __LINE__, flag); */
    /* log_info_hexdump(sync_param, 4); */
    switch (flag) {
    case 0:
        if (find_device_key_flag) {
...
void rcsp_adv_find_phone_handler(void)
{
    ...
    if (!find_phone_timer_flag) {
        log_info("time scan play find phone music\n");
