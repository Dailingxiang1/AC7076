...
int rcsp_make_set_adv_data(void)
{
    ...
    __this->bat_charge_L = sbox_left_charging_get();
    __this->bat_charge_R = sbox_right_charging_get();
    __this->bat_charge_C = sbox_box_charging_get();
    __this->bat_percent_L = sbox_battery_left_get();
    __this->bat_percent_R = sbox_battery_right_get();
    __this->bat_percent_C = sbox_battery_box_get();
...
void adv_role_switch_handle(u8 role)
{
    ...
    log_info("adv_role_switch_handle rcsp role change:%d, %d, %d\n", role, tws_api_get_role(), bt_rcsp_device_conn_num());
...
void screen_bt_ble_rcsp_send_info(void *p)
{
    ...
    u8 rcsp_conn_num = bt_rcsp_device_conn_num();
    // 判断是否已连接并进入RCSP通讯
    log_info("%s: %d %d %d", __func__, __LINE__, rcsp_conn_num, JL_rcsp_get_auth_flag(), __this->ble_adv_notify);
    if ((rcsp_conn_num > 0) && JL_rcsp_get_auth_flag() && __this->ble_adv_notify) {
        u8 data[18];
        update_adv_data(data);
        adv_info_notify(data, 18);
    }
