...
#else
#define TONE_RES_ROOT_PATH		 FLASH_RES_PATH   	//内置flash提示音根路径
#endif//TCFG_NOR_FS
#endif
...
struct app_protocol_private_handle_t {
    ...
    int (*tws_event_handler)(struct tws_event *event); //tws事件
    int (*bt_event_handler)(struct bt_event *event); //蓝牙事件
};
