...
#include "audio_demo.h"
#include "gpio.h"
#include "sin1k_sr44d1k_24bit_0db_std.c"

#define KEY_TEST	(IO_PORTB_02)
#define KEY_TEST2	0//(IO_PORTB_04)

extern struct audio_dac_hdl dac_hdl;
static u8 dac_demo = 0;

// 用于 DAC 播放的正弦波数据
const s16 data16_sin44100[441] = {
...
void audio_dac_demo_open(void)
{
    ...
    if (dac_hdl.channel == 2) {
        if (dac_hdl.pd->bit_width) {
            ...
            for (int i = 0; i < 882; i++) {
                data_addr32[i] = data32_sin44100[i / 2];
            }
            data_len = 441 * 4 * 2;
        } else {
            ...
            for (int i = 0; i < 882; i++) {
                data_addr16[i] = data16_sin44100[i / 2];
            }
            data_len = 441 * 2 * 2;
        }
    } else {
        if (dac_hdl.pd->bit_width) {
            data_addr32 = (s32 *)data32_sin44100;
            data_len = 441 * 4;
        } else {
            data_addr16 = (s16 *)data16_sin44100;
            data_len = 441 * 2;
        }
    }
